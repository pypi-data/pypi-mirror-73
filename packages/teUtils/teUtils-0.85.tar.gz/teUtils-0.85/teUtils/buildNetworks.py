...
def getLinearChain (lengthOfChain, rateLawType='MassAction', MaxKeq=5):
    ...
    ----------      
        ...
        arg2 (rateLawType String): Optional, can be 'MassAction' (default) or 'Michaelis'
             Michaelis generate reversible Miachaeli-Menten rate laws.  
  
        arg3 (MaxKeq float): Optional, maximum size of equilibrium constant
    ...
    Example
    -------  
    >>> # Generate linear chain of 5 steps with a maximum Keq of 6   
    >>> s = teUtils.buildNetworks.getLinearChain (5, 'MassAction', 6)
    >>>
    >>> s = teUtils.buildNetworks.getLinearChain (5, 'Michaelis', 6)
    >>> r = te.loada (s)
    >>> r.simulate (0, 50, 100)
    >>> r.plot()
    """
    ...
    if rateLawType == 'MassAction':
       for i in range (n):
           model += 'k' + str (i+1) + '0 = ' + str ('{:.2f}'.format (_random.random()*MaxKeq)) + ';  ' + \
...
def generateReactionList (nSpecies, nReactions):
    ...
    Return
    ------
        result: A nest list describing the reaction network
    ...
    Example
    -------
    >>> model = generateReactionList (6, 9)
    >>> print (model)     
    """
    reactionList = []
...
if __name__ == '__main__' :
   ...
   mod = getLinearChain ('MassAction', 9, MaxKeq=2)
   print (mod)
   r = _te.loada (mod)
