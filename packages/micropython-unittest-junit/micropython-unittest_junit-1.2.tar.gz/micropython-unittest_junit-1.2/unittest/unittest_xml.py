...
def run_class(cls, module, test_result):
    ...
    for name in dir(o):
        ...
        if name.startswith("test"):
            ...
            print("\n%s%s (%s) ...%s" % (bcolors.HEADER, name, cls.__qualname__, bcolors.ENDC), end="")
            m = getattr(o, name)
            try:
                set_up(name)
            except TypeError:
            ...
            try:
                ...
                xml_test_case.add_skipped_info(message=module.__file__, output=e.args[0])
...
class TestRunner:
    def run(self, suite):
        ...
        if res.failuresNum > 0 or res.errorsNum > 0:
            print(base.bcolors.FAIL + "FAILED " + base.bcolors.ENDC + "(failures=%d, errors=%d) " % (res.failuresNum, res.errorsNum))
        else:
            msg = "OK"
...
def main(module="__main__", noexit=False):
    def test_cases(m):
        for tn in dir(m):
            c = getattr(m, tn)
            if isinstance(c, object) and isinstance(c, type) and issubclass(c, base.TestCase):
                yield c
    try:
        ...
        if isinstance(module, str):
            if module == "__main__":
                module = ".".join(sys.argv[0].replace('\\', '/').split('/')[-1].split('.')[0:-1])
                print(module)
        else:
...
def unittest_all(noexit=False):
    ...
    def run_tests(path):
        ...
        for fname, ftype, *_ in uos.ilistdir(path):
            ...
            r = 0
            if is_file(ftype):
                if fname.lower().startswith("test_") and (fname.endswith(".py") or fname.endswith(".mpy")):
                    sys.path.insert(0, str(path))
                    try:
                        modname = fname[:fname.rfind(".")]
                        r = main(modname, noexit)
                    finally:
                        sys.path.pop(0)
            elif is_dir(ftype):  
                r = run_tests(fpath)
