...
eqs_na = """
I_Na = gnabar*m**3*h*(ENa-v) : amp
dm/dt=q10*(inf_m-m)/tau_m : 1
dh/dt=q10*(inf_h-h)/tau_h : 1
inf_m = 1./(1+exp(-(vu + 38.) / 7.)) : 1
inf_h = 1./(1+exp((vu + 65.) / 6.)) : 1
tau_m =  ((10. / (5*exp((vu+60.) / 18.) + 36.*exp(-(vu+60.) / 25.))) + 0.04)*ms : second
tau_h =  ((100. / (7*exp((vu+60.) / 11.) + 10.*exp(-(vu+60.) / 25.))) + 0.6)*ms : second
"""
...
eqs = """
dv/dt = (ileak + I_Na + ikht + iklt + ika + ih + ihcno + I)/C : volt
