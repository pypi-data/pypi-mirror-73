...
from meta1.block import Block, BlockHeader
...
def allow(ctx, foreign_account, permission, weight, threshold, account):
    ...
    if not foreign_account:
        ...
        foreign_account = format(
            PasswordKey(account, pwd, permission).get_public(), "META1"
        )
    print_tx(
        ctx.meta1.allow(
            foreign_account,
...
def history(ctx, account, limit, type, csv, exclude, raw):
    ...
    """
    from meta1base.operations import getOperationNameForId

    t = [["#", "time (block)", "operation", "details"]]

    for a in account:
        account = Account(a, meta1_instance=ctx.meta1)
...
def transfer(ctx, to, amount, asset, memo, account):
    """ Transfer assets
    """
    print_tx(ctx.meta1.transfer(to, amount, asset, memo=memo, account=account))
...
def newaccount(ctx, accountname, account, password):
    ...
    """
    print_tx(
        ctx.meta1.create_account(accountname, registrar=account, password=password)
    )
...
def upgrade(ctx, account):
    """ Upgrade account
    """
    print_tx(ctx.meta1.upgrade_account(account))
...
def cloneaccount(ctx, account_name, account):
    ...
    """
    from meta1base import transactions, operations
    ...
    op = {
        ...
        "extensions": {},
        "prefix": ctx.meta1.rpc.chain_params["prefix"],
    }
    op = operations.Account_create(**op)
    print_tx(ctx.meta1.finalizeOp(op, account, "active"))
...
def changememokey(ctx, key, account):
    ...
    """
    print_tx(ctx.meta1.update_memo_key(key, account=account))
...
def setproxy(ctx, proxy_account, account):
    ...
    """
    print_tx(ctx.meta1.set_proxy(proxy_account, account=account))
...
def unsetproxy(ctx, account):
    ...
    """
    print_tx(ctx.meta1.unset_proxy(account=account))
