...
class ComplexArguments:
    def __init__(self, sub_arg_objs: dict, with_group_prefix=False, conflict_warning=True):
        ...
        self._conflict_warning = conflict_warning
        self._arg_objs = {}
        self.__create_args()
    ...
    def __create_args(self):
        for sub, arg_obj in self._sub_arg_objs.items():
            ...
            self._arg_objs[sub] = Arguments(arg_obj, parser, self._with_group_prefix, self._conflict_warning)
...
class Arguments:
    def __init__(self, configObj, parser=None, with_group_prefix=False, conflict_warning=True):
    ...
    def _arg(self, field, v, parser, group=None):
        ...
        if parser.get_default(name) and self.conflict_warning:
            print('WARNING: duplicated argument defined on: ' + name)
            return self
