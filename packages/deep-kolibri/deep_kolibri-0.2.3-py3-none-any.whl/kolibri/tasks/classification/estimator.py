...
class Estimator(Component):
    ...
    defaults = {
        ...
        "explain":False,
        "sampler-algorithm": None,
    }

    def __init__(self, component_config=None, classifier=None, indexer=None):
        from kolibri.indexers import ClassificationIndexer
        ...
        if indexer is not None:
            ...
            self.indexer = ClassificationIndexer(multi_label=False)
        ...
        if not self.clf:
            raise Exception('The classification model: "' + str(self.component_config['models'])+'" could not be created.')
        self.sampler=None

        if self.component_config['sampler-algorithm']:
            self.sampler=get_sampler(self.component_config['sampler-algorithm'])
    ...
    def predict(self, X):
        ...
        if not predictions is None:
            ...
            return self.indexer.reverse_numerize_label_sequences(predictions)

        return self.indexer.reverse_numerize_label_sequences(self.clf.predict(X))

    def train(self, training_data, **kwargs):

        y=self.indexer.reverse_numerize_label_sequences([document.label for document in training_data])
    ...
    def process(self, document, **kwargs):
        ...
        if not self.clf:
            ...
            classes = self.indexer.reverse_numerize_label_sequences(np.ravel(class_ids))
...
#            self.clf.classes_ = self.indexer.la
    ...
    def persist(self, model_dir):
        ...
        return {"classifier_file": KOLIBRI_MODEL_FILE_NAME}
