...
from kolibri import config as k
...
class CONLL2003ENCorpus(FileStream):
    ...
    def _load_data(self):
        try:
            ...
            for line in raw_data:
                self.n_samples += 1
                for token in line:
                    self.target_values.add(token[self.target_columns])
    ...
    def __iter__(self):
        ...
        for token in self.raw_data:
            yield [d[self.content_column] for d in token], [d[self.target_columns] for d in token]
