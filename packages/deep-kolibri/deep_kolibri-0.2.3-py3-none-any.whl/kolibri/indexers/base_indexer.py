...
class BaseIndexer(object):
    ...
    def analyze_corpus(self,
        ...
        if len(self.token2idx) == 0 or force:
            self.build_token_dict(corpus, self.min_count)
        if len(self.label2idx) == 0 or force:
            self.build_label_dict(labels)

    def build_token_dict(self, corpus: List[List[str]], min_count: int = 3):
        """
    ...
    def build_label_dict(self, corpus: Union[List[List[str]], List[str]]):
