...
from kolibri.dnn.layers.crf import CRF
from kolibri.indexers.base_indexer import BaseIndexer
#from kolibri.dnn.tasks.classification.base_model import BaseClassificationModel
...
def load_indexer(model_path: str) -> BaseIndexer:
    """
    Load indexer from model
    When we using tf-serving, we need to use model's indexer to pre-process data
    Args:
        model_path:
    ...
    indexer: BaseIndexer = indexer_class(**indexer_info['config'])
    return indexer
