...
import numpy as np
import kolibri
from kolibri.dnn.tasks import get_model
    ...
    def __init__(self, component_config=None, model=None):
        ...
        if self.component_config['embeddings']=='default':
            self.embeddings = DefaultEmbedding(task=TaskType.CLASSIFICATION,
                                              sequence_length=self.component_config["sequence_length"], indexer=self.indexer)
        elif self.component_config['embeddings']=='word':
            self.embeddings = WordEmbedding(w2v_path=self.component_config["embedding_path"],task=TaskType.CLASSIFICATION,
                                              sequence_length=self.component_config["sequence_length"], indexer=self.indexer)

        self.clf=get_model(self.component_config['models'], embedding=self.embeddings, hyper_parameters=self.component_config)
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if self.component_config['class-weight']:
            class_weights = class_weight.compute_class_weight('balanced',
                                                              np.unique(y),
                                                              y)
            self.clf.fit(X, y,x_validate=X_val, y_validate=y_val, epochs=self.component_config["epochs"], callbacks=callbacks_list, fit_kwargs={"class_weight":class_weights})
        else:
            self.clf.fit(X, y,x_validate=X_val, y_validate=y_val, epochs=self.component_config["epochs"],  callbacks=callbacks_list)
