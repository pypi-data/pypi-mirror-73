...
from kolibri.dnn.tasks.text.classification.base_model import BaseClassificationModel
from kolibri.dnn.tasks.text.classification.dpcnn_model import DPCNN_Model
from kolibri.dnn.custom_loses import mean_false_error
...
class CNN_Model(BaseClassificationModel):
    ...
    def build_model_arc(self):
        ...
        if len(filter_sizes) >1:
            graph_in = L.Input(shape=(self.embedding.sequence_length,self.embedding.embedding_size ))
...
def get_model(model_type, embedding=None, hyper_parameters=None):
    ...
    return {
        'bilstm': BiLSTM_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'bigru':BiGRU_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'cnn':  CNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'cnn_lstm': CNN_LSTM_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'cnn_gru': CNN_GRU_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'avcnn':AVCNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'kmax_cnn': KMax_CNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'r_cnn': R_CNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'avrnn': AVRNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'dropout_bigru': Dropout_BiGRU_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'dropout_avrnn': Dropout_AVRNN_Model(embedding=embedding, hyper_parameters=hyper_parameters),
        'dpcnn' :DPCNN_Model(embedding=embedding, hyper_parameters=hyper_parameters)
...
if __name__ == "__main__":
    ...
    indexer = ClassificationIndexer(multi_label=False)

    embed = DefaultEmbedding(task=kolibri.config.TaskType.CLASSIFICATION, sequence_length=30, indexer=indexer)

    m = BiLSTM_Model(embed, {'loss': mean_false_error })
