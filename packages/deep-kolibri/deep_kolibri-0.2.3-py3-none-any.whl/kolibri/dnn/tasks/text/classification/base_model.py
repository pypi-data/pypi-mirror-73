...
class BaseClassificationModel(BaseModel):
    ...
    def __init__(self, embedding=None, hyper_parameters=None):

        super(BaseClassificationModel, self).__init__(embedding, hyper_parameters)
        if hyper_parameters is None and self.embedding.indexer.__getattribute__('multi_label') is True:
    ...
    def compile_model(self, **kwargs):
        ...
        if 'loss' in self.hyper_parameters:
            kwargs['loss'] = self.hyper_parameters['loss']
    ...
    def predict(self,
                x_data,
                batch_size=32,
                multi_label_threshold: float = 0.2,
                debug_info=False,
        ...
        """
        with kolibri.dnn.utils.custom_object_scope():
