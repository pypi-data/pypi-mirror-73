...
class BaseModel(object):
    ...
    @property
    def pre_indexer(self):
        warnings.warn("The 'pre_indexer' property is deprecated, "
                      "use 'indexer' instead", DeprecationWarning, 2)
        """Deprecated. Use `self.indexer` instead."""
    ...
    def fit_with_generator(self, generator_train, generator_validate=None,            batch_size: int = 64,
            epochs: int = 5,
            callbacks: List[keras.callbacks.Callback] = None,
            fit_kwargs: Dict = None,
            shuffle: bool = True):
        """
        Trains the model for a given number of epochs with generator function (iterations on a dataset).
    ...
    def compile_model(self, **kwargs):
        ...
        if not kolibri.config.config.disable_auto_summary:
...
if __name__ == "__main__":
    from kolibri.dnn.tasks import CNN_LSTM_Model
    from kolibri.dnn.corpus import ChineseDailyNerCorpus
