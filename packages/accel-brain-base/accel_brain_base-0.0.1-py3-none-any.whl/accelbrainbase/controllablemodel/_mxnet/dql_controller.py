    ...
    def learn(
        ...
        try:
            for n in range(iter_n):
                ...
                if (n + 1) % 100 == 0:
                    self.__logger.debug("Reward value(mean): " + str(reward_value_arr.mean().asnumpy()))
                    self.__logger.debug("Predicted Q-value(mean): " + str(predicted_q_arr.mean().asnumpy()))
                    self.__logger.debug("Real Q-value(mean): " + str(real_q_arr.mean().asnumpy()))
                    self.__logger.debug("Loss of Q-Value(mean): " + str(loss.mean().asnumpy()))
