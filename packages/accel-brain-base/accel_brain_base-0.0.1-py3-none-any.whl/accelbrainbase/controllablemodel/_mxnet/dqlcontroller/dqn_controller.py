...
class DQNController(DQLController):
    ...
    def select_action(
        ...
        Args:
            ...
            next_q_arr:                         Tensor of Q-Values in next time.
        ...
        if possible_meta_data_arr is not None:
            possible_meta_data_arr = possible_meta_data_arr[:, key_arr][:, 0]

        return (
            possible_action_arr[:, key_arr][:, 0], 
            possible_predicted_q_arr[:, key_arr][:, 0], 
            possible_reward_value_arr[:, key_arr][:, 0],
            next_q_arr[:, key_arr][:, 0],
            possible_meta_data_arr
        )
    ...
    def select_action_key(self, possible_action_arr, possible_predicted_q_arr):
        ...
        if epsilon_greedy_flag is False:
            key_arr = np.random.randint(
                size=(possible_action_arr.shape[1], ),
                low=0, 
                high=possible_action_arr.shape[1]
            )
        else:
            key_arr = possible_predicted_q_arr[:, :, 0].argmax(axis=1)
