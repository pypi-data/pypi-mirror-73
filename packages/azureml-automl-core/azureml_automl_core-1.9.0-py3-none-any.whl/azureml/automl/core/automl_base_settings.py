...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        self.path = path or (os.path.dirname(os.path.abspath(debug_log)) if debug_log else os.getcwd())

        self.iterations = iterations

        if primary_metric is None and task_type is None:
            raise ConfigException.create_without_pii('One or both of primary metric and task type must be provided.')
    ...
    def _verify_settings(self):
        ...
        if self.enable_onnx_compatible_models:
            ...
            if self.enable_dnn and self.task_type == constants.Tasks.CLASSIFICATION:
                msg = ('Classification task when enable_dnn is True is not ONNX compatible.')
                raise ConfigException.create_without_pii(msg)
