...
from azureml.automl.core.shared.exceptions import (MissingValueException,
...
class ForecastingParameters:
    ...
    def _validate_time_column_name(self) -> None:
        if self.time_column_name is None:
            raise MissingValueException.create_without_pii(
                'Timeseries need to set time column.', target="time_column_name",
    ...
    def _check_column_name_type(self, col_name: Any, target: str, reference_code: str) -> None:
        wrong_type_msg = "Wrong {} type: expected string, list of strings or None."
        if not isinstance(col_name, str):
            raise ScenarioNotSupportedException.create_without_pii(
                wrong_type_msg.format(target), target=target,
    ...
    def _check_target_lags_range(self, lag: int) -> None:
        if lag < 1 or lag > ForecastingParameters.MAX_LAG_LENGTH:
            raise OutOfRangeException.create_without_pii(
                "The {} must be between 1 and {} inclusive."
                .format(TimeSeries.TARGET_LAGS, ForecastingParameters.MAX_LAG_LENGTH),
                target=TimeSeries.TARGET_LAGS,
    ...
    def _validate_feature_lags(self) -> None:
        ...
        type_error_feature_lags = "Unsupported value of feature_lags, " \
                                  "which must be '{}' or '{}'.".format(*permitted_feature_lags_options)
        if self.feature_lags not in permitted_feature_lags_options:
            raise ScenarioNotSupportedException.create_without_pii(
                type_error_feature_lags,
                target=TimeSeries.FEATURE_LAGS,
    ...
    def _is_int_or_auto(val: Optional[Any], val_name: str, reference_code: str, allow_none: bool = True) -> None:
        ...
        if not isinstance(val, int) and val != TimeSeries.AUTO:
            raise ScenarioNotSupportedException.create_without_pii(
                "Unsupported value of {}. {} must be integer or '{}'.".format(val_name, val_name, TimeSeries.AUTO),
                target=val_name, reference_code=reference_code)
