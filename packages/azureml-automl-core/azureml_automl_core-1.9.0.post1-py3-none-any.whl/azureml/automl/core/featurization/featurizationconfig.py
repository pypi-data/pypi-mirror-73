...
class FeaturizationConfig:
    ...
    def _validate_dataset_language(dataset_language: str) -> None:
        if dataset_language not in TextDNNLanguages.supported:
            msg = "Invalid dataset_language specified. Please use one of these three character codes {}."
            msg = msg.format(TextDNNLanguages.supported)
            raise ConfigException.create_without_pii(
                msg, target="featurization_config.dataset_language",
                reference_code=ReferenceCodes._FEATURIZATION_CONFIG_VALIDATE_DATASET_LANGUAGE)
