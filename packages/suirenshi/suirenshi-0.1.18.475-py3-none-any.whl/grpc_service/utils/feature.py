...
class FeatureParameterize(object):
        ...
        for i in range(point_num):
            for j in range(i + 1, point_num):
                if self.result.labels[i] != self.result.labels[j]:
                    continue
                distance = np.sqrt(
                    np.power((center_point[i][0] - center_point[j][0]), 2) + 
                    np.power((center_point[i][1] - center_point[j][1]), 2))
    ...
    def _extract_feat(self, bbox, mask, limit_scale=100):
        ...
        feature_result = {'length': length,
                          ...
                          'max20brightness': top_brightness,
                          'min20brightness': low_brightness,
                          'gradients': gradients,
                          'contrast': contrast
                          }
        return feature_result
