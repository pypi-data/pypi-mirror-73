...
def model_popen(jobs_pid, script_path, base_path, model_id=-1, gpu_use=-1, port_use=0, model_use=None, cfg=None,
                model_path=None, model_module=None, auth_path=None, auth_module=None, auth_method=None, 
                decrypt_method=None):
    """
    ...
    cmd_pattern = '{python} {script_path} {base_path} {model_id} {cfg} {port_use} {model_use} ' \
                  '{model_path} {model_module} {auth_path} {auth_module} {auth_method} {decrypt_method}'
...
def pipeline_popen(jobs_pid, script_path, base_path, pipeline_id=-1, port_use=0, pipeline_use=None, cfg=None,
                   pipeline_path=None, pipeline_module=None, auth_path=None, auth_module=None, auth_method=None):
    """
    Popen to start process
    Args:
        script_path:
    ...
    """
    cmd_pattern = '{python} {script_path} {base_path} {pipeline_id} {cfg} {port_use} {pipeline_use} {pipeline_path} ' \
                  '{pipeline_module} {auth_path} {auth_module} {auth_method}'
    cmd = cmd_pattern.format(python=shlex_quote('python'),
                             script_path=script_path,
                             base_path=base_path,
...
def start_task(conf):
    ...
    for model_id in args.activate_models:
        ...
        logging.info(
            "Model {} using GPU: {}, PORT: {}, MODEL: {}".format(model_id, gpu_use, port_use, model_use))
        p = multiprocessing.Process(target=model_popen,
                                    args=(jobs_pid, model_script_path, base_path, model_id, gpu_use, port_use,
                                          ...
                                          args.auth_module, args.auth_method, args.decrypt_model_method))
        p.start()
    ...
    for pipeline_id in args.activate_pipelines:
        ...
        p = multiprocessing.Process(target=pipeline_popen,
                                    args=(jobs_pid, pipeline_script_path, base_path, pipeline_id, port_use, 
                                          pipeline_use, config_path, pipeline_path, args.pipeline_module, 
                                          args.auth_path, args.auth_module, args.auth_method))
        p.start()
    return jobs_pid
