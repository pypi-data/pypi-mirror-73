...
class KiWISTimeSeriesTransaction(TimeSeriesTransaction):
    ...
    def _add_metadata_params(
        ...
        metadata = list(
            itertools.chain.from_iterable(map(cls._convert_metadata, metakeys))
        )
    ...
    async def _get_time_series_list(
        ...
        params.setdefault(
            "returnfields", ["ts_id", "ts_name", "ts_path", "ts_shortname"]
        )
    ...
    async def _commit_ts_read(self, op: TimeSeriesTransaction.DataOperation) -> None:
        if op.path:
            time_series_list = await self._get_time_series_list(
                op.path, params=op.kwargs
            )
        else:
            time_series_list = await self._get_time_series_list(
                id_list=[op.kwargs["id"]], params=op.kwargs
            )
        try:
    ...
    async def _commit_ts_filter(self, op: TimeSeriesTransaction.DataOperation) -> None:
        op.time_series_iterable = await self._get_time_series_list(
            op.filter, params=op.kwargs
        )

    async def _commit_metadata_read(
        self, op: TimeSeriesTransaction.DataOperation
    ) -> None:
        ...
        if op.time_series._is_ensemble():
            ...
            for ens_ts in ens_ts_list:
                ...
                for row in j["data"]:
                    ...
                    member_ts_data.append(
                        [t0_data, dispatch_info_data, op.member, ts_member, data_member]
                    )
                ...
                return
            raise KeyError(f"Ensemble trace not found for t0={op.t0}, member={op.member}, dispatch_info={op.dispatch_info}")
        else:
