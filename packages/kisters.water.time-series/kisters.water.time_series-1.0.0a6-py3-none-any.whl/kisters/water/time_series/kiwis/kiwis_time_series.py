...
from kisters.water.time_series.core.time_series import TimeSeries, EnsembleMemberInfo
...
class KiWISTimeSeries(TimeSeries):
    ...
    def transform(self, transformation: str) -> TimeSeries:
        if re.match(".*\\(.*\\)", self.path):
            ...
            return self.store._get_time_series(
                path="tsm(" + self.path + ");" + transformation
            )
    ...
    def ensemble_members(
        ...
        for ens_ts in ens_ts_list:
            ...
            ens_members = [
                {"t0": t0, "dispatch_info": dispatch_info, "member": member}
