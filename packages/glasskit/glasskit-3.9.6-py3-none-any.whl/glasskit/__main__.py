...
def copy_sources(src_dir, dst_dir, appname):
    ...
    for srcname in os.listdir(src_dir):
        ...
        if srcname == "__pycache__" or srcname.endswith(".pyc"):
            continue
        ...
        if os.path.isdir(srcname):
            ...
            if srcname.endswith(".tmpl"):
                dstname = dstname[:-5]
                filemode = os.stat(srcname).st_mode & 0x1ff
...
def run_new_model(args):
    ...
    if project_name is None:
        raise RuntimeError("project name is not detected. are you inside your project folder?")

    model_name = args.model_name[0]
    class_name = ""
    for i, sym in enumerate(model_name.lower()):
        class_name += sym.upper() if i == 0 else sym
    ...
    model_filename = os.path.join(project_name, f"models/{model_name}.py")
    os.makedirs(os.path.dirname(model_filename), exist_ok=True)
    with open(model_filename, "w") as modelf:
        modelf.write(tmpl.render({
...
def run_new_controller(args):
    ...
    tmplfilename = os.path.join(glasskit_directory(), "unit_templates/controller.py.tmpl")
    with open(tmplfilename) as tmplfile:
        tmpl = jinja2.Template(tmplfile.read())
...
def main():
    ...
    parser_create.add_argument("--venv", "-v", type=str, default="venv", help="venv directory name")

    parser_new = subparsers.add_parser("new", help="create a new unit, i.e. model, controller, task")
    parser_new_subparsers = parser_new.add_subparsers(dest="unit", required=True, help="unit-type help")
    new_model_parser = parser_new_subparsers.add_parser("model", help="create a new model")

    new_model_parser.add_argument("model_name", nargs=1, type=str, help="model_name")
    new_model_parser.add_argument("fields", nargs="*", type=str, help="field descriptors: field1:any field2 "
                                                                      "field3:string "
                                                                      "field4:list etc...")

    new_task_parser = parser_new_subparsers.add_parser("task", help="create a new task")
    new_task_parser.add_argument("class_name", nargs=1, type=str, help="name of the task class")
    new_task_parser.add_argument("fields", nargs="*", type=str, help="task fields list")

    new_controller_parser = parser_new_subparsers.add_parser("controller", help="create a new controller")
    new_controller_parser.add_argument("ctrl_path", nargs=1, type=str, help="controller prefix path, i.e. api/v1/users")
    new_controller_parser.add_argument("--disable-auth", "-d", action="store_true",
                                       help="disable auth requirement")
    new_controller_parser.add_argument("--class", "-c", dest="ctrl_class", type=str,
