...
class Field:
    ...
    def __init__(self, required=False, rejected=False, restricted=False,
                 default=None, index=None, unique=False):
...
class TypedField(Field):
    ...
    def validate(self, value):
        ...
        if not isinstance(value, self.TYPE):
            raise ValidationError(f"field {self.name} must be an instance of type {self.TYPE.__name__}")
...
class StringField(TypedField):
    ...
    def __init__(self, min_length=None, max_length=None, re_match=None, auto_trim=True, **kwargs):
    ...
    def validate(self, value):
        ...
        if self.min_length is not None and len(value) < self.min_length:
            raise ValidationError(f"field {self.name} minimum length violated")

        if self.max_length is not None and len(value) > self.max_length:
            raise ValidationError(f"field {self.name} maximum length violated")

        if self.re_match is not None and not self.re_match.match(value):
            raise ValidationError(f"field {self.name} must match pattern \"{self.re_match.pattern}\"")
