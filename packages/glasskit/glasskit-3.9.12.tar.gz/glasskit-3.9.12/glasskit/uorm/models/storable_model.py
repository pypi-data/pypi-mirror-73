...
class StorableModel(BaseModel):
    ...
    def find(cls, query=None, **kwargs):
        if not query:
            query = {}
        return db.meta.get_objs(cls._ctor, cls.collection, cls._preprocess_query(query), **kwargs)
    ...
    def find_projected(cls, query=None, projection=('_id',), **kwargs):
        if not query:
            query = {}
        return db.meta.get_objs_projected(cls.collection, cls._preprocess_query(query),
                                          projection=projection, **kwargs)

    @classmethod
    def find_one(cls, query, **kwargs):
        return db.meta.get_obj(cls._ctor, cls.collection, cls._preprocess_query(query), **kwargs)
    ...
    def _cache_get(cls, cache_key, getter, ctor=None):
        ...
        if db.l1c.has(cache_key):
            ...
            td = time() - t1
            log.debug("%s L1 hit %s %.3f secs", db.l1c.NAME, cache_key, td)
            return ctor(data)
        ...
        if db.l2c.has(cache_key):
            ...
            td = time() - t1
            log.debug("%s L2 hit %s %.3f secs", db.l2c.NAME, cache_key, td)
            return ctor(data)
        ...
        td = time() - t1
        log.debug("%s miss %s %.3f secs", db.l2c.NAME, cache_key, td)
        return obj
    ...
    def _invalidate(cache_key):
        if db.l1c.delete(cache_key):
            log.debug("%s delete %s", db.l1c.NAME, cache_key)
        if db.l2c.delete(cache_key):
            log.debug("%s delete %s", db.l2c.NAME, cache_key)
