...
def interpolate(
    ...
    """ 
    Approximates data using function, then interpolates for timestamps between start and end with specified frequency.
    
    Args:
        data (pd.DataFrame or pd.Series): Time series to impute.
        kind (str): Specifies the kind of interpolation (‘linear’, 'stepwise', ‘nearest’, ‘zero’, ‘slinear’, ‘quadratic’,
        ...
        outside_fill (str): Specifies how to fill values outside input data range ('NaN' or 'extrapolate').
            Default behaviour is to raise a ValueError if no outside_fill method is specified.
    ...
    for ts in pd.DataFrame(data).columns:
        ...
        if outside_fill is None:
            ...
            interper = scipy.interpolate.interp1d(
                x_observed, y_observed, kind=kind, bounds_error=False, fill_value=outside_fill
            )
