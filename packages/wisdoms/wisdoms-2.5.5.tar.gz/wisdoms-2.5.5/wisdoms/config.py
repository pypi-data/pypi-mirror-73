...
class Config:
    ...
    def __init__(self, layer=4):
        ...
        for i in range(layer):
            ...
            if env:
                try:
                    self.configuration.update(find_file(read_config, ms_path, 'common ms config'))
                except Exception as e:
                    print('load global config failed viz. ms.yml')
                    print(e)
                break
            else:
            ...
            for i in range(layer):
                ...
                if configuration:
                    try:
                        ...
                        print('Load local config failed viz. dev.yml, prod.yml')
                        print(e)
                    break
                else:
