...
from saliency_detector.networks.joint_poolnet import build_model, weights_init
...
class Solver(object):
    def __init__(self, train_loader, test_loader, config):
        ...
        self.net = None
        self.build_model() # assigns to self.net
        if config.mode == 'test':
    ...
    def reset_iterator(self):
        self.test_loader_it = iter(self.test_loader)
    ...
    def __next__(self):
        n_images = len(self.test_loader)
        if self.image_index+1 < n_images:
            self.image_index += 1
            return self.predict()
        else:
            raise StopIteration
    ...
    def predict(self, img=None, color_space='BGR'):
        ...
        time_e = time.time()
        logging.info('Saliency Prediction Speed: %f FPS' % (1 / (time_e - time_s)))
        return multi_fuse
        ...
        for i, data_batch in enumerate(self.test_loader):
            ...
            if test_mode == 0:
                ...
                with torch.no_grad():
                    ...
                    cv2.imwrite(os.path.join(self.config.test_fold, name[:-4] + '_' + mode_name[test_mode] + '.png'), multi_fuse)
        time_e = time.time()
        print('Saliency Prediction Speed: %f FPS' % (img_num/(time_e-time_s)))
        print('Finished Saliency Prediction')
