...
from lgblkb_tools.geometry.utils.geom_utils import GenericGeometry, get_from_geojson, line_xy, pathify, \
    is_clockwise, generate_cells, min_dist
from .line_utils import TheLine
from .point_utils import ThePoint
from .utils.katana import katana
from .utils.plot_utils import plot_polygon, plot_patches
from .utils.tsp import SimpleTSP
...
class ThePoly(GenericGeometry):
    ...
    def synthesize(cls, cities_count, poly_extent=1000, hole_count=0, seed=None, hole_cities_count=None):
        ...
        while True:
            ...
            try:
                unholy_field = cls.from_tsp(cities_count=cities_count, seed=seed,
                                            city_locator=lambda ps: ps * poly_extent)
                ...
                for i in range(1000):
                    ...
                    hole = cls.from_tsp(cities_count=hole_cities_count or np.random.randint(4, 10), seed=seed,
                                        city_locator=lambda ps: ps * np.random.randint(poly_extent * 0.2,
                                                                                       poly_extent * 0.5) +
                                                                np.random.randint(poly_extent * 0.1, poly_extent * 0.2,
                                                                                  2))
    ...
    def get_exterior_lines(self, as_the_line=True, show=0):
        lines = list()
        if as_the_line:
            line_getter = lambda _line: TheLine(_line)
        else:
            line_getter = lambda _line: _line
    ...
    def katanize(self, threshold, show=0, **kwargs):
        ...
        if show:
            for p in polygons: plot_polygon(p, **kwargs)
        return polygons
    ...
    def generate_clusters(self, grid_resolution, n_clusters, show='', random_state=1):
        ...
        final_centers = kmeans.cluster_centers_.copy()
        for i, center in enumerate(kmeans.cluster_centers_):
            if not ThePoint(center).geometry.within(self.geometry):
                grid_points = interior_points[kmeans.labels_ == i]
                distances = cdist(kmeans.cluster_centers_[i].reshape(-1, 2), grid_points, )
                closest_grid_point = grid_points[np.argmin(distances)]
                final_centers[i] = closest_grid_point

        if show:
            plot_clusters, plot_self = 1, 0
            if len(show) == 1:
                plot_clusters = int(show)
            elif len(show) == 2:
                plot_clusters, plot_self = [int(x) for x in show]
    ...
    def get_connectivity_graph(self, as_the_poly=True, show=''):
        ...
        if as_the_poly:
            polymaker = lambda x: x.polygon
        else:
            polymaker = lambda x: x
        connections = list()
    ...
    def __contains__(self, item):
        if isinstance(item, ThePoint):
            ...
            return self.geometry.contains(item.geometry) or self.geometry.intersects(
                item.geometry) or self.geometry.touches(item.geometry)
