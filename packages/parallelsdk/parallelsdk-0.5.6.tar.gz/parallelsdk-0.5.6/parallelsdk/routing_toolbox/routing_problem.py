...
class RoutingProblem:
    ...
    def set_engine_type(self, engine_type):
        if not isinstance(engine_type, RoutingEngineType):
            err_msg = "RoutingProblem - SetEngineType: invalid engine type " + type(engine_type)
            logging.error(err_msg)
            raise Exception(err_msg)
    ...
    def create_distance_matrix(self, data):
        ...
        for i in range(q):
            ...
            response = self.send_request(origin_addresses, dest_addresses, API_key)
        ...
        self.distance_matrix_rows = len(self.distance_matrix)
        self.distance_matrix_cols = len(self.distance_matrix[0])
        self.distance_matrix_mult = 1

    def send_request(self, origin_addresses, dest_addresses, API_key):
    ...
    def infer_distance_matrix(self, metric="euclidean", API_key=''):
        if metric == 'maps':
            data = {"addresses": [], "API_key": API_key}
            for depot in self.depots:
                depot.get_address(API_key)
                data["addresses"].append(depot.get_address_for_geocoding())
                break
            for location in self.locations:
                location.get_address(API_key)
                data["addresses"].append(location.get_address_for_geocoding())
            self.create_distance_matrix(data)
        else:
