...
class Definition:
	...
	def validate(self, results, field_fqn, field_name, definition, value, dependent_values):
		try:
			...
			if value is None:
				nullable = False
				nullable = Index.is_nullable(self, field_name)
				if nullable is True:
...
class Index:
	...
	def is_nullable(definition_or_name, field_name):
		nullable = False
		if Config.definition_name_resolver.is_nullable(Index.as_dict(), field_name):
			nullable = True
		else:
			definition = Index.get(definition_or_name)
