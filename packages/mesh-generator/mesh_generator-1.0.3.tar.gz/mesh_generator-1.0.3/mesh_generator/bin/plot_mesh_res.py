"""
Plotting the fortran data results saved in the file mesh_res.txt.
"""
import numpy as np
...
def plot_mesh_res(adjusted_mesh: dict, save_plot: bool, show_plot: bool, label: str):
    """
       plot the mesh points in mesh_res.txt
    ...
    axs[0].plot(xvals, yvals, label="Legacy mesh")
    # plot the mesh ratio
    ...
    for i in range(0, 2):
        for segments in adjusted_mesh['segment_list']:
            ...
            axs[0].hlines(max(segments['ds0'], segments['ds1']), s0, s1, linewidth=3, label=seg_label)
    ...
    if save_plot:
        file_path = '../../plots/'
        file_name = str(time.ctime()) + " " + title
