...
def custom_connection_class(inner_wrapper, module_path):
    class CustomConnection(CustomConnectionBase):
        _self_sqreen_cursor_class = custom_cursor_class(
            inner_wrapper, module_path)
        _self_sqreen_wrapper = functools.partial(
...
def custom_connect(inner_wrapper, module_path, original_connect):
    ...
    """
    cls = custom_connection_class(inner_wrapper, module_path)

    def wrapper(wrapped, instance, args, kwargs):
        return cls(
            inner_wrapper(module_path, "connect")(wrapped, None, args, kwargs))
