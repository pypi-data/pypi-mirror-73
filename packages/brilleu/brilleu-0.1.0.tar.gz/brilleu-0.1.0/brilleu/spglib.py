...
class BrCell:
    ...
    def isapprox(self, other):
        return brille.Direct(self.lat).isapprox(brille.Direct(other.lat))
...
class BrSpgl:
    ...
    def get_primitive_transform(self):
        if not self.spglib_used:
            return None
        pt = brille.PrimitiveTransform(self.spglib_dict['hall_number'])
    ...
    def get_input_Direct(self):
        return brille.Direct(self.get_input_basis(), self.get_input_atom_positions(), self.get_input_atom_index(), self.hall)
    def get_conventional_Direct(self):
        return brille.Direct(self.get_conventional_basis(), self.get_conventional_atom_positions(), self.get_conventional_atom_index(), self.hall)

    def get_input_BrillouinZone(self):
        return brille.BrillouinZone(self.get_input_Direct().star)
    def get_conventional_BrillouinZone(self):
        return brille.BrillouinZone(self.get_conventional_Direct().star)
