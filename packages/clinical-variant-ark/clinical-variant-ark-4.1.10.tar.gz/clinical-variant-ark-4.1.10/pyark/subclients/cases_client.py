...
from protocols.protocol_7_4.cva import ReportEventType, Transaction
import logging
import pandas as pd
...
class CasesClient(cva_client.CvaClient):
    ...
    def get_clinical_report_version(self, case_id, case_version, report_id, report_version, as_data_frame=False):
        results, _ = self._get(
            "{endpoint}/{case_id}/{case_version}/{report_id}/{report_version}".format(
                endpoint="clinical-reports",
                case_id=case_id,
        ...
        if not results:
            logging.warning(
                "No clinical report found with id-version {}-{} report_id-version {}-{}".format(
                    case_id,
                    case_version,
                    report_id,
                    report_version
