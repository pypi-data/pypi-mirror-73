...
class CvaClient(RestClient):
    ...
    def _build_next_page_params(headers):
        next_page_params = {}
        limit = headers.get(CvaClient._LIMIT_HEADER, headers.get(CvaClient._LIMIT_HEADER.lower(), None))
        marker = headers.get(CvaClient._MARKER_HEADER, headers.get(CvaClient._MARKER_HEADER.lower(), None))
        if marker:
