...
def simp2trad(text=None,text_list=None,opencc_type="s2tw"):
    '''
    用來將簡體中文轉成繁體中文
    參數包含：
    text : 單一要轉換的文本，型別須為string；對應回傳值也為string
    text_list : 有多個要轉換的文本，型別須為list，list中每個元素為一文本string；對應回傳值也為list
    opencc_type : 套件OpenCC轉換簡繁體時的模式，默認為"s2tw"
    '''
    
    cc = OpenCC(opencc_type) 
