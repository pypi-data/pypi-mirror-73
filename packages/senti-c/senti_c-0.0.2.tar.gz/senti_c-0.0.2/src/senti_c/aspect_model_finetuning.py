...
from .utils import AspectProcessor, aspect_convert_examples_to_features as convert_examples_to_features, evaluate_ote, evaluate_ts, ASPECT_LABEL_MAP, SENTIMENT_LABEL_MAP, chg_labels_to_aspect_and_sentiment


logger = logging.getLogger(__name__)  

DEFAULT_ASPECT_CLASSIFICATION_PRETRAIN_MODEL = 'bert-base-chinese'  
...
class AspectSentimentModel:
    '''屬性情感分類模型的訓練：使用設定的參數來訓練/微調預測模型'''  
    
    def __init__(self,
                 model_name_or_path=DEFAULT_ASPECT_CLASSIFICATION_PRETRAIN_MODEL,
                 config_name="",
                 tokenizer_name="",
                 cache_dir="",
                 no_cuda=False,
        ...
        self.model = MyNewBertForTokenClassification.from_pretrained(
            model_name_or_path,
    ...
    def _train(self,saved_args,train_dataset):  
        ...
        for _ in train_iterator:  # 保留epoch的tqdm
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                tr_loss += loss.item()
                if (step + 1) % saved_args["gradient_accumulation_steps"] == 0:
    ...
    def _evaluate(self, model, saved_args, prefix=""):
        ...
        eval_dataset, all_evaluate_label_ids = self._load_and_cache_examples(saved_args, evaluate=True) 
        ...
        eval_loss = 0.0
        aspect_eval_loss = 0.0
        sentiment_eval_loss = 0.0
        nb_eval_steps = 0
        
        aspect_preds = None
        sentiment_preds = None
        aspect_out_label_ids = None
        sentiment_out_label_ids = None
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "token_type_ids" : batch[2], "labels": batch[3]}
                outputs = model(**inputs)
                tmp_eval_loss, tmp_aspect_loss,tmp_sentiment_loss, aspect_logits, sentiment_logits = outputs[:5]

                if self.n_gpu > 1:
                    tmp_eval_loss = tmp_eval_loss.mean()  
                    tmp_aspect_loss = tmp_aspect_loss.mean()
                    tmp_sentiment_loss = tmp_sentiment_loss.mean()
            ...
            if aspect_preds is None:
                ...
                aspect_labels, sentiment_labels = chg_labels_to_aspect_and_sentiment(inputs["labels"])
                aspect_out_label_ids , sentiment_out_label_ids = aspect_labels.detach().cpu().numpy(), sentiment_labels.detach().cpu().numpy()
                ...
                aspect_out_label_ids = np.append(aspect_out_label_ids, aspect_labels.detach().cpu().numpy(), axis=0)
                sentiment_out_label_ids = np.append(sentiment_out_label_ids, sentiment_labels.detach().cpu().numpy(), axis=0)

                
        eval_loss = eval_loss / nb_eval_steps
        aspect_eval_loss = aspect_eval_loss / nb_eval_steps
        sentiment_eval_loss = sentiment_eval_loss / nb_eval_steps

        aspect_preds = np.argmax(aspect_preds, axis=2)
        sentiment_preds = np.argmax(sentiment_preds, axis=2)
        
        ## 以下計算評估標準 ##
        aspect_out_label_list = [[] for _ in range(aspect_out_label_ids.shape[0])]
        aspect_preds_list = [[] for _ in range(aspect_out_label_ids.shape[0])]

        # 以下兩者是為了去計算整體ABSA評估標準所用
        absa_out_label_list = [[] for _ in range(sentiment_out_label_ids.shape[0])]
        absa_preds_list = [[] for _ in range(sentiment_out_label_ids.shape[0])]
        
        for i in range(aspect_out_label_ids.shape[0]):
            evaluate_label_ids = all_evaluate_label_ids[i] #取得該句子中有效的位置索引
            for j in range(aspect_out_label_ids.shape[1]):
                if j in evaluate_label_ids: 
                    asp_true = ASPECT_LABEL_MAP[aspect_out_label_ids[i][j]]
                    asp_pred = ASPECT_LABEL_MAP[aspect_preds[i][j]]
                    sent_true = SENTIMENT_LABEL_MAP[sentiment_out_label_ids[i][j]]
                    sent_pred = SENTIMENT_LABEL_MAP[sentiment_preds[i][j]]

                    aspect_out_label_list[i].append(asp_true)
                    aspect_preds_list[i].append(asp_pred)

                    # 將屬性標記和情感標記標籤結合
                    absa_out_label_list[i].append(asp_true+"-"+sent_true)
                    absa_preds_list[i].append(asp_pred+"-"+sent_pred)
        
        ote_scores = evaluate_ote(gold_ot=aspect_out_label_list, pred_ot=aspect_preds_list)
        ts_scores = evaluate_ts(gold_ts=absa_out_label_list, pred_ts=absa_preds_list)

        results = {
            "loss": eval_loss,
            "aspect_loss": aspect_eval_loss,
            "sentiment_loss": sentiment_eval_loss,
        }
        results.update(ote_scores)
        results.update(ts_scores)
    ...
    def train(self,
              ...
              best_model_metric='absa_micro_f1',
        ...
        參數包含：
        data_dir : 放置訓練檔案與驗證檔案的資料夾路徑，注意訓練與驗證檔案須為txt格式，且檔案中內容要符合格式規定(一列包含字、屬性-情感標記，中間用空白隔開；不同資料以空行隔開)
        output_dir : 放置輸出模型與相關檔案的輸出路徑
        max_seq_length : 斷字後最大的長度，默認"128"
        ...
        best_model_metric : 判斷驗證時最佳模型的評估標準，默認為"absa_micro_f1"，也可指定其他參數："aspect_precision"/ "aspect_recall" / "aspect_f1" / "absa_macro_neg" / "absa_macro_neu" / "absa_macro_pos" / "absa_macro_f1"/ "absa_micro_precision" / "absa_micro_recall"
        ...
        learning_rate : Adam優化器的初始學習率，默認為"5e-5"
        ...
        ## 獲取訓練資料dataset ##
        train_dataset, _ = self._load_and_cache_examples(saved_args,evaluate=False)
        ...
        if do_eval and self.local_rank in [-1, 0]:
            ...
            for checkpoint in checkpoints:
                ...
                model = MyNewBertForTokenClassification.from_pretrained(checkpoint)
            ...
            if save_best_model:
                ...
                model = MyNewBertForTokenClassification.from_pretrained(best_name)
                model_to_save = (
                    model.module if hasattr(model, "module") else model
                ) 
