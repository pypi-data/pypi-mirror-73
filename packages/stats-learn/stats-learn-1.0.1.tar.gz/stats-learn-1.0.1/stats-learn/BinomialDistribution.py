...
class Binomial(Distribution):
    ...
    def replace_stats_with_data(self):
        ...
        """
        n = len(self.data)  # calculates the total number of trials
        positive = 0  # calculate the number of positive trials
        for i in self.data:
            if i == 1:
                positive += 1

        p = positive / n  # calculates the probability of success
        self.n = n
        self.p = p
    ...
    def pdf(self, k):
        ...
        mean = self.mean
        sigma = self.stdev
        ojo = factorial(n) / (factorial(k) * factorial(n - k))
        tobi = (p ** k) * (1 - p) ** (n - k)
    ...
    def __add__(self, other):
        ...
        result.stdev = sqrt(self.stdev ** 2 + other.stdev ** 2)
