...
try:
    ...
    if not os.path.exists('rcnn'):
        download_url('1-5IDPWueGSqvI7zSLyXuDcrUGQEIMv0G', name = 'rcnn')
    print ("[INFO] Run once again")
...
class RetinaFace:
    def __init__(self, config, nms=0.4, nocrop=False, decay4=0.5, vote=False):
        ...
        Args:
            ...
            nms (float, optional): [description]. Defaults to 0.4.
            nocrop (bool, optional): [description]. Defaults to False.
            decay4 (float, optional): [description]. Defaults to 0.5.
            vote (bool, optional): [description]. Defaults to False.

        Returns:
            None

        Examples:
            RetinaFace().get_input()
            RetinaFace().detect()
        """
    ...
    def get_input(self, img):
        ...
        if (self.pixel_scale != 1.0) or (self.pixel_means[0] != 0.0) or (self.pixel_means[1] != 0.0) or (
                self.pixel_means[2] != 0.0) \
                or (self.pixel_stds[0] != 1.0) or (self.pixel_stds[1] != 1.0) or (self.pixel_stds[2] != 1.0):
            for i in range(3):
                im_tensor[0, i, :, :] = (img[:, :, 2 - i] / self.pixel_scale - self.pixel_means[2 - i]) / \
                                        self.pixel_stds[2 - i]
                data = nd.array(im_tensor)
        else:
            # preprocessing steps
            image_RGB = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            data = image_RGB.transpose(2, 0, 1)
