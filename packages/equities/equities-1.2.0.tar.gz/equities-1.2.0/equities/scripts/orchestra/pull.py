...
from lib.pull.sec.Universe import Universe as Gu
from pull.sec.Company  import Company  as Gc
from pull.sec.waiter   import waiter   as Gw

from pull.yahoo.Universe import Universe as Yu
from pull.yahoo.Company  import Company  as Yc
from pull.yahoo.waiter   import waiter   as Yw 
...
from access.DataAccessor import DataAccessor 
...
class pull(object):
    ...
    def execute_pipeline(self,test = False,multi_process = False,quarters = [],num_companies=None):
        total_stages = 12
        ...
        print(pipeline_str(':beers:','downloading packages',2))
        if quarters == []:
        ...
        if num_companies != None:
            ciks = gu.get_ciks()[:num_companies]
        else:
            ciks = gu.get_ciks()[:num_companies]
        self.dump_sec(gu,ciks,epoch = 10,multi_process=False)

        ### YAHOO DATA PULL SECTION BEGIN ###

        #print(pipeline_str(':aerial_tramway:','DataAccessor.Resolver (cik -> ticker)',5))
        #res_map = self.da.resolver.resolve(ciks,'cik','ticker') 
        #tickers = list(res_map.values())            

        #print(pipeline_str(':beers:','Download Yahoo Data with Waiter',6))
        #self.config['yahoo']['ticks'] = tickers               
        #yw = Yw(data_config = {'yahoo':self.config['yahoo']})  
        #yw.download_data(tickers = tickers)                   

        #print(pipeline_str(':boom:','Expanding Yahoo Universe',7))
        #yu = Yu(tickers = tickers)
        #yu.build()

        #print(pipeline_str(':memo:','Dumping Yahoo Universe',8))
        #self.dump_yahoo(yu,tickers = tickers)

        ### YAHOO DATA PULL SECTION END ###

        print(pipeline_str(':sweat_drops:','Cleaning Directories',12))
        self._clean_dirs()

        #print(pipeline_str(':aerial_tramway:','Resolving data...',13))
        #prop_df = self.da.get(item = 'properties')
        #company_names = [(cik,prop_df[int(cik)]['name']) for cik in prop_df.columns]

        #print(pipeline_str(':beers:','Download Wikipedia Data with Waiter',9))
        #self.config['wikipedia']['company_names'] = company_names               
        #ww = Ww(data_config = {'wikipedia':self.config['wikipedia']})
        #ww.download_data()

        #print(pipeline_str(':memo:','Dumping Wikipedia Text',8))
        #for cik in ciks:
        #    try:
        #        wc = Wc(cik = cik)
        #        wc.to_txt()
        #    except:
        #        #print('Could not get: %s'%str(cik))
        #        pass
    ...
    def dump_sec(self,u,ciks,epoch = 10,multi_process=False):
        ...
        if multi_process:
            ...
            to_remove_ciks = []
            for cik in tqdm(ciks,desc = '\tdone'.ljust(40),ncols=self.ncols):
                c = Gc(cik = cik, u = u)
                try:
