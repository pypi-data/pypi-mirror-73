from passportsdk.traits.auth import AuthTrait
from passportsdk.traits.admin import AdminTrait
...
class AppClient(object):
    ...
    def admin_user_list(self, user_token):
        ...
        [后台]用户列表
        """
        return AdminTrait.admin_user_list(self, user_token)

    def admin_user_info(self, user_token, user_id):
        """
        [后台]用户信息
        """
        return AdminTrait.admin_user_info(self, user_token, user_id)

    def admin_user_modify(self, user_token, user_id, modify_user_info):
        """
        [后台]用户编辑(创建)
        """
        return AdminTrait.admin_user_modify(self, user_token, user_id, modify_user_info)
