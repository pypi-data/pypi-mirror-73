        ...
        if os.path.isfile('vnm.yml'):
            with open('vnm.yml', 'r') as f:
                ...
                while version in self.yaml_upgrades:
                    log.warning('Updating vnm.yml to version %d...', version+1)
        ...
        if os.path.isfile('vnm.state'):
            with open('vnm.state', 'r') as f:
                ...
                while version in self.state_upgrades:
                    ...
                    version = self.getStateVersion(docs)
                    log.warning('Updated vnm.state to version %s...', version)
                statedata = docs[1]
...
def _cmd_upgrade_config(args):
    ...
    if os.path.isfile('pip.json'):
        ...
        log.warning('Upgrading vnm.yml...')
        if not os.path.isfile('vnm.yml.bak'):
            log.info('Backing up vnm.yml to vnm.yml.bak...')
            shutil.copy('vnm.yml', 'vnm.yml.bak')
        else:
            log.info("vnm.yml.bak exists, skipping backup.")
        log.info('Reading...')
        cfg.load()
    else:
        log.critical('Neither vnm.yml nor pip.json are present.  Exiting.')
        sys.exit(1)
