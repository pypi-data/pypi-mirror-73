...
class SmartTagsConvertHelper:
    ...
    def __kb_heartbeat_detection__(self):
        ...
        """
        try:
            res = requests.get(url='http://{}:9075/kb/health-check'.format(self.kwargs['interface']['kb_ip']),
                               timeout=10)
    ...
    def GetDateFromText(self, date_str):
        ...
        if date_str:
            for reg in self.kwargs['date_reg']:
    ...
    def __kb_interface_call(self, route, data):
        ...
        """
        try:
            res = requests.post(url='http://{}:9075/kb{}'.format(self.kwargs['interface']['kb_ip'], route), json=data,
                                timeout=self.kwargs.get('timeout', 20)).json()
            if res['success']:
                return res['data']
        except:
            return None
    ...
    def GetRelatedDivision(self, address):
        ...
        if res:
            for item in res:
                result[item['originvalue']] = item.get('result', '')
        return result
    ...
    def GetFullOrgName(self, agency: list, is_callback=False):
        ...
        if agency:
            type_dict = collections.OrderedDict((key, value) for key, value in self.kwargs['tv_area'].items() if value)
    ...
    def PinyinIsSimilarity(self, x, y):
        """
        判断两个词拼音是不是相似
        """
        ...
        return self.edit(pinyin_x, pinyin_y) > self.kwargs.get('PinyinSimilarity', 0.8)
