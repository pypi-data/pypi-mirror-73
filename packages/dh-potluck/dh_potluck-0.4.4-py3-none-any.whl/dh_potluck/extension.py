    ...
    def init_app(self, app, **kwargs):
        ...
        def handle_server_error(error):
            ...
            if query_params:
                ...
                tracer.set_tags({'http.query_string': urlencode(clean)})
            ...
            tracer.set_tags({'http.content_length': request.content_length})

            if request.content_length > MAX_BODY_SIZE:
                tracer.set_tags({'http.body': 'Body too large, content could not be logged.'})
                return error_response
            ...
            if body:
                ...
                tracer.set_tags({'http.body': json.dumps(clean)})
            else:
                # If we can't parse as JSON, log the raw body
                body = request.get_data(as_text=True)
                tracer.set_tags({'http.body': body})
