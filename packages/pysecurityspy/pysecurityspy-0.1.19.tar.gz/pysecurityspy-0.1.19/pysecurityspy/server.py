...
class Events:
    ...
    def _connect(self):
        ...
        self.event_data.update({0:{"is_motion": False}})
        self.event_data.update({1:{"is_motion": False}})
        self.event_data.update({2:{"is_motion": False}})
        self.event_data.update({3:{"is_motion": False}})
                    ...
                    if data[:14].isnumeric():
                        ...
                        if event_id in EVENT_TYPES:
                            ...
                            if event_id == EVENT_TYPE_TRIGGER_M:
                                self.event_data[uid]["is_motion"] = True
                            elif event_id == EVENT_TYPE_FILE:
                                self.event_data[uid]["is_motion"] = False
                            
                            if self.event_callback:
                                self.event_callback(self.event_data)
