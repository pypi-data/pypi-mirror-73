...
def test_build():
    if os.path.exists(".localfunk.yaml"):
        os.remove(".localfunk.yaml")

    sam.build("127.0.0.1:5000", template_path="test/fixtures/template.yml")

    assert os.path.exists(".localfunk.yaml")
...
def test_get_functions():
    ...
    assert sam.get_functions(template_yaml=template_yaml) == {
        "FunctionOne": {"code_uri": "app1", "file": "handler1", "function": "handle"},
        "FunctionTwo": {"code_uri": "app2", "file": "handler2", "function": "do"},
    }
