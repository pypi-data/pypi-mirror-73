...
class AutoMLConfig(object):
                ...
                if whitelist_models is None:
                    if all_models is not None:
                        ...
                        if set(all_model_names).issubset(set(blacklist)):
                            raise ConfigException.create_without_pii(
                                "Cannot find models to train, all available models are in blacklist")
                else:
                    ...
                    if blacklist is not None and set(whitelist_models).issubset(set(blacklist)):
                        raise ConfigException.create_without_pii(
                            "Cannot find models to train, all whitelisted models are also in blacklist")
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        self._validate_streaming_whitelisting(is_timeseries, compute_target)
    ...
    def _validate_streaming_whitelisting(self, is_timeseries: bool, compute_target: str) -> None:

        def _handler(scenario: str, should_raise: bool) -> None:
            error_msg = "'whitelist_models' consists of unsupported algorithms for {} scenarios."

            if should_raise:
                logger.error(error_msg.format(scenario))
                raise ConfigException.create_without_pii(error_msg.format(scenario))
        ...
        if whitelisted_models:
            ...
            wlm_all_streaming = set(whitelisted_models).issubset(streaming_supported_algorithms)
            wlm_some_streaming = set(whitelisted_models).intersection(streaming_supported_algorithms)
