...
laserController = laser.Controller(protocol=protocol, print_position_in_relative=False)

laserController.move_to_well(plate["A1"])

print("Moving to start position...")
laserController.move_z(plate["A1"].top(5).point.z)
...
def show_or_exit(key):
    ...
    if key in ("I", "i"):
        ...
        laserController.move_to_well(plate["A12"])
        laserController.move_z(plate["A12"].top(5).point.z)

    if key in ("O", "o"):
        save_offset()
        top_of_well = plate["H12"].top().point
        laserController.move_to_well(plate["H12"])
        laserController.move_z(plate["H12"].top(5).point.z)
...
u - save calibration and move to well A1
i - save calibration and move to well A12
o - save calibration and move to well H12
s - save calibration 
h - home
q - exit
""")
...
3. Press i to move laser mount over A12
4. Move laser mount so that bottom of metal ferule is centered and level with top of well A12
5. Press i to move laser mount over H12
6. Move laser mount so that bottom of metal ferule is centered and level with top of well H12
