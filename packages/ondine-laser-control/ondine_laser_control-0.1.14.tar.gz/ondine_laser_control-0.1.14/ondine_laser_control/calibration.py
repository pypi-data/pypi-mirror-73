...
def show_or_exit(key):
    ...
    if key in ("O", "o"):
        top_of_well = plate["F1"].top().point
        laserController.move_to_well(plate["F1"])
        laserController.move_z(plate["F1"].top(5).point.z)

    if key in ("P", "p"):
        top_of_well = plate["F12"].top().point
        laserController.move_to_well(plate["F12"])
        laserController.move_z(plate["F12"].top(5).point.z)
