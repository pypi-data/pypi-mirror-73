...
def set_button_purple(protocol_to_use: ProtocolContext):
    protocol_to_use._hw_manager.hardware._backend.gpio_chardev.set_button_light(red=True, blue=True, green=True)
...
def set_button_off(protocol_to_use: ProtocolContext):
    protocol_to_use._hw_manager.hardware._backend.gpio_chardev.set_button_light(red=False, blue=False, green=True)
        ...
        while not threading.current_thread().stopped():
            ...
            if shouldFlash:
                if time.time() % 2 == 0:
                    ...
                    set_button_off(protocol_to_use)
...
def show_or_exit(key):
    ...
    if key in ('k', 'K', "shift up"):
        laserController.move_relz(movement_distance)

    if key in ('m', 'M', "shift down"):
        laserController.move_relz(-movement_distance)
    if key == "up":
