...
from azureml.studio.modules.ml.common.metrics_logger import BaseMetricsLogger, BinaryClassificationMetricsLogger, \
    ClusterMetricsLogger
...
class EvaluateModelModule(BaseModule):
    ...
    def evaluate_generic(cls, scored_data: DataTable, scored_data_to_compare=None, output_metrics=True):
        ...
        if output_metrics:
            try:
                if task_type == TaskType.BinaryClassification:
                    BinaryClassificationMetricsLogger.log_metrics(report_data, report_data_to_compare)
                elif task_type == TaskType.Cluster:
                    ClusterMetricsLogger.log_metrics(result_df, result_df_to_compare)
                else:
                    BaseMetricsLogger.log_metrics(result_df, result_df_to_compare)
