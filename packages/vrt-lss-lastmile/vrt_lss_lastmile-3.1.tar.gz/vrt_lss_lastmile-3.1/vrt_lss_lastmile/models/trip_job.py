...
class TripJob(object):
    ...
    def job_type(self):
        ...
        Возможные типы работ:   * `LOCATION_ARRIVAL` - прибытие на локацию (завершение парковки, начало разрешенного временного окна для работы)   * `READY_TO_WORK` - исполнитель готов к работе (завершение ожидания временного окна для работы, начало ожидания открытия локации и начала временного окна заявки)   * `START_WORK` - начало работы на локации (завершение ожидания открытия локации и начала временного окна заявки, начало работы)   * `FINISH_WORK` - конец работы на локации (завершение работы, начало ожидания оптимального времени отъезда на следующую локацию)   * `LOCATION_DEPARTURE` - отъезд с локации   # noqa: E501
