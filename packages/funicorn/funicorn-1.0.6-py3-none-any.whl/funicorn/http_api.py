...
class HttpAPI(threading.Thread):
    def __init__(self, funicorn_app, host='0.0.0.0', port=5001, stat=None, threads=40, name='HTTP', timeout=1000, debug=False, register_conn=True):
        ...
        def predict_url():
            ...
            try:
                ...
                if 'url' in request.args:
                    ...
                    if results is ResponseStatus.CANNOT_DOWNLOAD_URL:
                        raise DownloadURLError(message='Cannot download data from url!')
                    else:
                        final_res = results
                    resp = jsonify({
