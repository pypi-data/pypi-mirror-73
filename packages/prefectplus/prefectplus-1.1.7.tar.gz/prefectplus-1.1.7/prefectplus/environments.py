...
class K8sDaskJobEnvironment(Environment):
        ...
        try:
            ...
            with open(
                ...
                with prefect.context(secrets=secrets):
                    ...
                    executor_cls = DaskExecutor(local_processes=self.local_processes, **self.executor_kwargs)
    ...
    def _load_spec_from_file(self) -> dict:
        ...
        if self.job_spec_file:
            with open(self.job_spec_file) as job_spec_file:
                ...
                #job["metadata"][
                #        "name"
                #    ] = f"{job['metadata']['name']}-{str(uuid.uuid4())[:8]}"
        return job