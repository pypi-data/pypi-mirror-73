...
def pivot_category_prices(prices_df):
    prices_df = pd.pivot_table(
        ...
        index=["shipment_id"],
        columns=["category"],
        values=["price_in_eur", "initial_price_in_eur", "purchase_price_in_eur"],
    )
    ...
    prices_df = prices_df.rename(
        columns={
            ...
            "purchase_price_in_eur_customs": "customs_purchase_price",
            ...
            "purchase_price_in_eur_other": "other_purchase_price",
        }
    )
    return prices_df.drop(["initial_price_in_eur_customs", "initial_price_in_eur_other"], axis=1)
