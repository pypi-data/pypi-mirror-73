...
class CAEN14xxETCtrl(HC_Backend):
    def __init__(
    ...
    def check_connection(self):
        ...
        if self.connection_type == VISA:
            ...
            try:
                self.device.query("$BD:00,CMD:MON,PAR:BDNAME")
            ...
            try:
                self.query("$BD:00,CMD:MON,PAR:BDNAME")
            except Exception as e:
                logger.warning(
                    "Device connection check fail. Setting online to false",
                    exc_info=True,
                )
                return False
            else:
    ...
    def update_setting(self, setting: str, value):
        ...
        if setting_X == "CHX_enable":
            if value == "True":
                self.write(f"$BD:00,CMD:SET,CH:{channel},PAR:ON")
            else:
                self.write(f"$BD:00,CMD:SET,CH:{channel},PAR:OFF")
            status = self.read_status(channel)

            if status is None:
                return "None"
            return str(bool(status["on_off"]))

        elif setting_X == "CHX_I_set":
            ...
            self.write(f"$BD:00,CMD:SET,CH:{channel},PAR:ISET,VAL:{value}")
            ...
            self.write(f"$BD:00,CMD:SET,CH:{channel},PAR:VSET,VAL:{value}")
