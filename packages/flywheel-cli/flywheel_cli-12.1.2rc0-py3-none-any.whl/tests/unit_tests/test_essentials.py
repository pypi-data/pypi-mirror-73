...
def test_get_permission_level(cont, uid, result):
    assert essentials._get_permission_level(cont, uid, Mock()) == result


def test_get_permission_level_with_role_ids():
    sdk_mock = Mock()
    ...
    assert essentials._get_permission_level(cont, "foo", sdk_mock) == "role1, role2"
