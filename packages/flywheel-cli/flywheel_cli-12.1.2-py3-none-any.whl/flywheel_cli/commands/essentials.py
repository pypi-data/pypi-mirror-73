...
def ls(args):
    ...
    for child in result.children:
        table.append(_get_row_for_container(child, parent, user, args.ids, fw))

    console.print_table(sys.stdout, table)


def _get_row_for_container(
    cont, parent, user, show_ids, fw
):  # pylint: disable=too-many-branches
    if user is None:
        level = "admin"
    elif "permissions" in cont:
        level = _get_permission_level(cont, user.id, fw)
    else:
        level = _get_permission_level(parent, user.id, fw)
...
def _get_permission_level(cont, uid, fw):
    if cont is not None:
        for perm in cont.get("permissions", []):
            ...
            if perm.id == uid and getattr(perm, "role_ids", None):
                ...
                return ", ".join(_get_role_names(perm.role_ids, fw))
...
def _get_role_names(role_ids, fw):
