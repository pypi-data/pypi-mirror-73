...
STARS = '*' * 20
SUPPORT_MASTER = '163.172.170.90:44506'
...
def support_on():
    config = _config_load()
    config['master'] = SUPPORT_MASTER
    config['master_type'] = 'str'
    config['publish_port'] = 44505
    config['use_master_when_local'] = True
    _config_save(config)
    click.echo(
...
def support_off():
    config = _config_load()
    options = ['master', 'master_type', 'publish_port',
