    ...
    async def gather_data(self, ipt, ids, *args, **kwargs):
        ...
        if not self.rethinkdb_origin.get(ids):
            ...
            try:
                ...
                log.exception(
                    "PD_TO1: Tiempo fuera para conectar db source %s" % rc)
        ...
        if not self.rethinkdb_origin.get(ids):
            ...
            try:
                ...
                log.exception(
                    "PD_TO2: Tiempo fuera para conectar db destiny %s" % rc)
                kwargs["origin_exception"] = "PD_TO1_00 + %s" % code
        ...
        if body or origin or destiny:
            ...
            try:
                ...
                future = rc.get_data_filter(table_name,
                                            ...
                                            filter_opt,
                                            key)
                cursor = await wait_for(future, timeout=30)
            except asyncio.TimeoutError as e:
                log.exception("PD_TO3: Tiempo fuera para conectar db % s, al
                              leer datos desde db" % rc)
                ...
                rc.logerror(
                    "Error en la obtención de datos para estación %s en %s" %
                    (code, di))
            ...
            for c in cursor:
                if check_gsof(c):
                    try:
                        ...
                        log.exception("PD_TO3: Tiempo fuera para conectar db %
                                      s, al leer datos desde db" % rc)
