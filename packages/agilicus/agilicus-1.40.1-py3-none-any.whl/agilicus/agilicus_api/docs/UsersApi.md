...
> ListMFAChallengeMethods list_challenge_methods(user_id, limit=limit, challenge_type=challenge_type)
...
challenge_type = 'sms' # str | challenge method type query (optional)
    ...
    try:
        # Get all of a user's multi-factor authentication challenge method configuration
        api_response = api_instance.list_challenge_methods(user_id, limit=limit, challenge_type=challenge_type)
        pprint(api_response)
    except ApiException as e:
