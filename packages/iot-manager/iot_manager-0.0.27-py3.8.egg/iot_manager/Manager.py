...
from gevent import Greenlet, get_hub
...
class Manager:
    def __init__(self, ssl_context: ssl.SSLContext = None, host: str = "127.0.0.1", connection_port: int = 8595,
        ...
        self.__executor = get_hub().threadpool
    ...
    def __connection_listener(self):
        while True:
            ...
            # catch any SSL errors and let the user know about them so they can fix them on the client end
                ...
                self.logger.error("(Connection Listener) Error occurred in the connection_listener greenlet. Exception: "
                                  + str(exception))
    ...
    def __thread_handler(self):
        ...
        self.__background_greenlets.append(Greenlet(run=self.__connection_listener))
        self.__background_greenlets.append(Greenlet(run=self.__heartbeat_checker))
        self.__background_greenlets.append(Greenlet(run=self.__message_listener))

        # logging output
        self.logger.info("(Greenlet Handler) Starting greenlets.")

        # start each greenlet in the pool
        for let in self.__background_greenlets:
            let.start()

        # logging output
        self.logger.info("(Greenlet Handler) All greenlet started.")
        return
