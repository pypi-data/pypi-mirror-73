...
def request_telemetry_definition(i2c_master: Union[I2CMaster, SupMCUMaster],
    ...
    if "(on STM)" in version or "(on QSM)" in version:
    ...
    return SupMCUTelemetryDefinition(name_response.items[0].value,
                                     ...
                                     False if not ("(on STM)" in version or "(on QSM)" in version) else simulatable_response.items[0].value)


def get_name_from_version(i2c_master: I2CMaster, address: int) -> str:
    """
