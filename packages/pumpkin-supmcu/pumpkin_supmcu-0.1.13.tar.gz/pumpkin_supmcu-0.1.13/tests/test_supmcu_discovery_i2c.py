...
def _gen_tel_def_request(addr, cmd_name, idx, data_types):
    ...
    expected_write_values.insert(-1, f'SUP:TEL? 0\n'.encode('ascii'))
    expected_values = list(zip_longest([], expected_write_values, fillvalue=addr))
    ...
    responses = [fmt_resp, name_resp, length_resp, version_resp, simulatable_resp]
