...
import numpy as np
from pycbp.node import VarNode
from pycbp.utils import (Message, diff_max_marginals,
                         engine_loop)
from pycbp.utils.np_utils import (nd_expand, nd_multiexpand,
                                  reduction_ndarray)
from pycbp.configs.base_config import baseconfig
...
class BaseGraph():  # pylint: disable=too-many-instance-attributes
    ...
    def add_varnode(self, node):
        """add one `~pycbp.node.VarNode` to this graph, idx follow the increasing
        order
