...
class ExponentialSmoothing:
            ...
            for model_name, model_obj in kwargs.items():              
                if model_name == best_model:                    
                    if best_model == 'ann-hw':
                        ...
                        timeseries, _, _ = self.transform_ts_log_difference_offset(timeseries)            
                        model_obj = holtwinters.SimpleExpSmoothing(timeseries).fit(optimized=True)
    ...
    def compute_forecast_intervals(
        ...
        >>> import datup as dt
        >>> sm=dt.ExponentialSmoothing()
        >>> sm.compute_forecast_intervals(timeseries,forecast,model_name='model_name',model,interval)
        """
        try:    
            ci_upper = []
            ci_lower = []
    ...
    def ets_forecast(
        ...
        >>> import datup as dt
        >>> sm=dt.ExponentialSmoothing()
        >>> sm.ets_forecast(timeseries,model_name='model_name',model)
        """
        try:      
            ...
            forecast_intervals = self.compute_forecast_intervals(timeseries, df_prediction, model_name[0], model, interval=interval)                          
        except IOError as error:
            print(f'Exception found: {error}')
            raise
