...
class HydrogenTechnicalEvaluation(TechnicalEvaluation):
    ...
    def __init__(self, data: HydrogenData, config: GeneralAnalysisConfig, path: str):
        ...
        self.__electrolyzer = PemElectrolyzerMultiDimAnalytic(16000, electrolyzer_data_config=ElectrolyzerDataConfig())
    ...
    def degradation_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=HydrogenState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data=data.resistance_increase_el, label=HydrogenState.RESISTANCE_INCREASE_EL, color=PlotlyPlotting.Color.RED))
        yaxis.append(Axis(data=data.exchange_current_dens_decrease_el, label=HydrogenState.EXCHANGE_CURRENT_DENS_DECREASE_EL, color=PlotlyPlotting.Color.GREEN))
        plot.lines(xaxis, yaxis, [1])
        self.extend_figures(plot.get_figures())
    ...
    def soh_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=HydrogenState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data=data.soh_el, label=HydrogenState.SOH_EL))
    ...
    def efficiency_el_plotting(self):
        ...
        yaxis.append(Axis(data=cell_efficiency_array_end, label='cell efficiency end', color=PlotlyPlotting.Color.RED))
