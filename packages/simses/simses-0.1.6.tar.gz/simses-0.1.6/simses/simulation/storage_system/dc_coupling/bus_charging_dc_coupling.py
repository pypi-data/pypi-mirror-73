...
from simses.simulation.storage_system.dc_coupling.load.dc_bus_charging import DcBusCharging


class BusChargingDcCoupling(DcCoupling):

    def __init__(self, charging_power: float, generation_power: float):
        super().__init__(DcBusCharging(charging_power), PvDcGeneration(generation_power))
