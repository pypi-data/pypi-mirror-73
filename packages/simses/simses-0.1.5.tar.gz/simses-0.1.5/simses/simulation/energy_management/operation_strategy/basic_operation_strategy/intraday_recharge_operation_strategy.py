...
class IntradayRechargeOperationStrategy(OperationStrategy):
    ...
    def next(self, time: float, system_state: SystemState, power: float = 0) -> float:
        total_time = time - self.__simulation_start
        step = np.int(total_time / self.__timestep)
        ...
        soc_max = (total_capacity - self.__fcr_reserve * self.__max_fcr_power) / total_capacity - (1 -
                self.__soc_max_system)
        if (step + 1) % self.__IDM_TRANSACTION_TIME == 0 and\
