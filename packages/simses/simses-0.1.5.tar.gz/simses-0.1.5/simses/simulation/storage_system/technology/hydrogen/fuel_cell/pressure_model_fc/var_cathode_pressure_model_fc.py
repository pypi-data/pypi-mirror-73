...
class VarCathodePressureModelEl(PressureModelEl):
    ...
    def __init__(self, pressure_controller: PressureControllerEl):
        super().__init__()
    ...
    def calculate(self, time, hydrogen_state: HydrogenState):
        ...
        n_h2_prod = hydrogen_state.hydrogen_production * delta_time  # mol
        ...
        self.p_c_1 = p_c_0 + PressureModelEl.IDEAL_GAS_CONST * stack_temp / self.volume_gas_separator * (1 + x_h2o_c) * \
                     (n_h2_prod - self.n_h2_out_c ) * 10 ** (-5)  # bar

        # new pressure anode
        self.p_a_1 = p_a_0 + PressureModelEl.IDEAL_GAS_CONST * stack_temp / self.volume_gas_separator * (1 + x_h2o_a) * \
