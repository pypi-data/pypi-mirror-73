"""
===============
Dataset (:mod:fepydas.datatypes.Dataset)
==============

This modules provide classes that represent commonly encountered types of spectroscopic data
...
class Dataset(BaseDataset):
  ...
  def applyVacuumCorrection(self, temperature=20, pressure=101325, humidity=0, co2=610):
    """
    Applies the vacuum correction to the axis values. See `fepydas.datatypes.Data.VacuumCorrection` for details.
    """
  ...
  def convertToEnergy(self, unit="eV"):
    """
    Converts axis from wavelength to energy and applies Jacobian transformation on data
    """
    self.axis.convertToEnergy(unit)
    self.data.applyJacobianTransformation(self.axis)

  def antiBloom(self, over, width=1, reverse=False, ignore=[]):
    """
    Extends overrange into adjacent pixels to make sure they will be ignored in further analysis.

    Parameters
    ----------

    over: float
      The threshold for what is considered overrange

    width: int, optional
      The number of pixels for which to extend the overrange (i.e. number of pixels that are expected to be affected by blooming)

    reverse: boolean, optional
      if True, the processing will be made from the back (this may be necessary if the data direction has been reversed from the readout direction previously)

    ignore: list, optional
      a list of indexes to ignore. Note: this does not do anything for `Data1D`, for `Data2D` and `Data3D` it refers to the indexes of the keys axis.
    """
    if type(self.data) is Data1D:
      self.data.antiBloom(over, width=width, reverse=reverse)
    else:
      self.data.antiBloom(over, width=width, reverse=reverse, ignore=ignore)
...
class Spectrum(Dataset):
  ...
  def export(self,filename):
    """
    Exports the Spectrum to a text file.
  ...
  def identifyPeaks(self, threshold=10, width=10):
    """
    This function identifies Peaks within the Spectrum, e.g. for use in a fitting procedure (see fepydas.workers.Fit.CalibrationFit)
    The peaks are identified by differentiating twice.
    
    Parameters
    ----------
    threshold : float, optional
      The threshold signal/noise to consider something a peak
      
    width : int, optional
       The expected half width of peaks 
       
    Returns
    ---------
    array 
      A list of index ranges where peaks have been found
  ...
  def sort(self):
    order = numpy.argsort(self.axis.values)
    self.axis.values = self.axis.values[order]
    self.data.values = self.data.values[order]
...
class PLE(SpectrumSeriesWithCalibration):
  """
  A PLE dataset, including the spectra as a function of excitation wavelength, a calibration and the lamp response
  """  
  ...
  def applyLampResponseCorrection(self,zeroFloor=False):
    """
    Applies the measured Lamp Response to the Data
    
    Parameters
    -----------
    zeroFloor: bool, optional
      Whether the minimum response value should be treated as 0.
  ...
  def calibrateLamp(self,width=2,thresh=1):
    """ 
    This function assumes the PLE data contains only the Lamp itself and is used to calibrate the excitation axis
    Uses `fepydas.workers.Fit.LimitedGaussianFit.initializeAutoLimited`
    ...
    Returns
    --------
    `fepydas.datatypes.Data.Transformation`
  ...
  def toPlot(self):
    """
    Produce the default plot
    
    Returns
    ----------
    `fepydas.constructors.Plots.ContourPlot` with log=True
    """    
