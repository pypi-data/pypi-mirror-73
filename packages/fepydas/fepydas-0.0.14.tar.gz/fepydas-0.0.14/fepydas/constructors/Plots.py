"""
========================================
Plots (:mod: fepydas.constructors.Plots)
========================================

This module provides constructors for common plot types using matplotlib.
...
from numpy import pi, linspace, log10, ndarray
from pylab import *
...
class Plot1D(BasePlot):
  def __init__(self, axis: Data1D, polar=False):
    ...
    if polar:
      axis.values = axis.values/360*2*pi
    self.setXAxis(axis)
...
class SpectrumScatterPlotWithFit(MultiSpectrumScatterPlot):
  def __init__(self, spectrum: Spectrum, fit: Fit, interpol=False):
    super().__init__([spectrum],labels=[""])
    if interpol:
      axis = linspace(spectrum.axis.values[0],spectrum.axis.values[-1],1000)
      self.plotLine(axis, fit.evaluate(axis))
    else:
      self.plotLine(spectrum.axis.values, fit.evaluate(spectrum.axis.values))
...
class ContourPlot(Plot2D):
  def __init__(self, spectra: SpectrumSeries, log = False):
    ...
    if log:
      values = log10(spectra.data.values+0.001)
    else:
...
class ContourMapPlot(Plot2D):
  def __init__(self, map: Map, log = False, colors=["black","red","green","blue","white"], zRange=None):
    ...
    if log:
      values = log10(map.data.values.T+0.001)
    else:
