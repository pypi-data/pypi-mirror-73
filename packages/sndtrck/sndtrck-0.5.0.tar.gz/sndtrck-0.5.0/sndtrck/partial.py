...
class Partial:
    ...
    def split_at_jump(self, pitchdev: float) -> list[Partial]:
        """
        Splits this partial at the points where there is a jump
        between two breakpoints where the pitch deviation 
        is higher than `pitchdev`

        NB: this pitch deviation is time dependent, allowing
            for sparsely sampled partials

        pitchdev: pitch deviation (in midinotes) per time
        """
        groups = []
        a = self.toarray()
        row1 = a[0]
        ...
        for i in range(1, len(a)):
            ...
            f0 = row0[1]
            f1 = row1[1]
            dev = (f2m(f1) - f2m(f0)) / (row1[0] - row0[0])
            if dev > pitchdev:
                # copy rows, this row not inclusive
                groups.append(a[lastidx:i])
                lastidx = i
        if lastidx < len(a) - 1:
            groups.append(a[lastidx:])
        partials = [Partial.fromarray(group) for group in groups]
        return partials
...
def concat2(partials: list[Partial], fade=0.005) -> Partial:
    """
...
def concat(partials:list[Partial], fade=0.005) -> Partial:
    """
    Concatenate multiple Partials to produce a new one.
    Assumes that the partials are non-overlapping and sorted
...
def _breakpoint_deviates(b0: Breakpoint, b1: Breakpoint, b2: Breakpoint, 
                         freqdelta: float, ampdelta: float, bwdelta: float) -> bool:
    t0, f0, a0, bw0 = b0
    t, f, a, bw = b1
    t1, f1, a1, bw1 = b2
    dt = (t - t0) / (t1 - t0)
...
def _note_deviates(b0: Breakpoint, b1: Breakpoint, b2: Breakpoint, pitchdelta: float, dbdelta: float) -> bool:
    """
...
def _partial_snap_times(partial: Partial, dt: float) -> Partial: 
    """
