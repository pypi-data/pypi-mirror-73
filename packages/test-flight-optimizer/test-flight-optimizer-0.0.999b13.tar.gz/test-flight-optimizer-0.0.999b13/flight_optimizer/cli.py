...
def search(departure, destinations, show_all_destinations, suggest_options):
    try:
        calculator = FlightOptimizer(departure, destinations)
        response = calculator.process(show_all_destinations, suggest_options)
        click.echo(response)
...
class FlightOptimizer:
    ...
    def process(self, show_all_destinations, suggest_options):
        departure_city = self._get_city(self.departure)
        destination_cities = list(map(self._get_city, self.destinations))
    ...
    def _get_city(city: str) -> City:
        ...
        if city.lower() != city_name.lower():
            ...
            correct_name_options = set(map(lambda possible_city: possible_city['name'], cities[1:]))
        else:
            is_found = True
            valid_name = city_name
            assumed_name = None
            correct_name_options = None
