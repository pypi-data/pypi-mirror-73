...
def search(departure, destinations, show_all_destinations, suggest_options):
    try:
        ...
        flights = flight_optimizer.process(show_all_destinations, suggest_options)
        # click.echo(response)

        des = []
        for f in flights:
            a = {'city': f.destination.city.valid_name,
                 'airport': f.destination.name,
                 'price': f.price,
                 'distance': f.distance,
                 'price_per_km': f'{f.price_per_kilometer:.2f}',
                 'is_reachable': f.is_found}
            if not f.is_found:
                a = {'city': f.destination.city.valid_name,
                     'is_reachable': f.is_found}
            des.append(a)
            print(a)
        ressss = {'departure': {'city': f.departure.city.valid_name,
                        'airport': f.departure.name}, 'destination': des}
        print(ressss)
...
class FlightOptimizer:
    ...
    def process(self, show_all_destinations, suggest_options):
        ...
        # filtered_flights = list(filter(lambda x: x.price_per_kilometer < float('inf'), sorted_flights))
