...
from ._compat import check_thread_support, queue
...
class BackgroundWorker(object):
    ...
    def kill(self):
        # type: () -> None
        print("background worker got kill request")
        with self._lock:
            if self._thread:
    ...
    def flush(self, timeout, callback=None):
        ...
        print("background worker got flush request")
        with self._lock:
            if self.is_alive and timeout > 0.0:
                self._wait_flush(timeout, callback)
        print("background worker flushed")
    ...
    def _wait_flush(self, timeout, callback):
        ...
        if not self._timed_queue_join(initial_timeout):
            ...
            print("%d event(s) pending on flush", pending)
