...
class ImagingSystem(logging.LoggerMixin, plotting.Plottable):
    ...
    *Principles of Optics*, Pergamon Press (Oxford UK)
    ...
    def load_zernikes(self, zernike_coefs, kind='OSA'):
        """
        Loads zernike coefficients represented as a dict: `{zernike_i: val}`.
        The coefficients are expected to be in RADIANS!

        :param zernike_coefs: zernikes to be loaded
        :type zernike_coefs: dict
        :param kind: the way zernikes are represented (`mn` or 'OSA`)
        :type kind: str
        :raises: ValueError
        ...
        **Example**
        ...
        if kind == 'OSA':
            ...
            self._zernike_coefs = {
                zernikes.mn_to_j(m, n): coef for (m, n), coef in
                zernike_coefs.items()
            }
        else:
            raise ValueError('Only `OSA` and `mn` indexing are available!')
    ...
    def _apply_aberrations(self, H):
        ...
        wf = zernikes.construct_wavefront_from_js(
            self._zernike_coefs, rho, theta
        )
