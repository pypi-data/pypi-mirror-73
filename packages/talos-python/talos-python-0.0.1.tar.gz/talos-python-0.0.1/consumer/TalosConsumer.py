...
#
from thrift.topic.ttypes import GetDescribeInfoRequest
from thrift.topic.ttypes import TopicTalosResourceName
from admin.TalosAdmin import TalosAdmin
from client.TalosClientConfig import TalosClientConfig
from client.TalosClientFactory import TalosClientFactory
from utils import Utils
from utils.Utils import ReadWriteLock
from consumer.MessageProcessorFactory import MessageProcessorFactory
from consumer.MessageReaderFactory import MessageReaderFactory
from thrift.consumer.ttypes import ConsumeUnit
from thrift.consumer.ttypes import QueryWorkerRequest
from thrift.consumer.ttypes import RenewResponse
from thrift.consumer.ttypes import RenewRequest
from thrift.consumer.ttypes import LockWorkerRequest
from client.TalosErrors import InvalidArgumentError
...
class TalosConsumer(object):
    ...
    def check_partition_task(self):
        try:
            ...
            logger.error("Exception in CheckPartitionTask: ", throwable)
    ...
    def check_worker_info_task(self):
        try:
            ...
            logger.error("Get worker info error: ", throwble)
    ...
    def renew_task(self):
        ...
        while maxRetry > 0:
            try:
                ...
                logger.error("Worker: " + self.workerId + " renew error: ", e)
                continue
            maxRetry -= 1
    ...
    def check_and_get_topic_info(self, topicTalosResourceName=None):
        topicName = Utils.get_topic_name_by_resource_name(str(topicTalosResourceName.topicTalosResourceName.topicTalosResourceName))
        response = self.talosAdmin.get_describe_info(GetDescribeInfoRequest(topicName))

        if not topicTalosResourceName.topicTalosResourceName.topicTalosResourceName == response.topicTalosResourceName.topicTalosResourceName:
            logger.info("The consumer initialize failed by topic not found")
            raise InvalidArgumentError("The topic: " + str(topicTalosResourceName.topicTalosResourceName.topicTalosResourceName) + " not found")
