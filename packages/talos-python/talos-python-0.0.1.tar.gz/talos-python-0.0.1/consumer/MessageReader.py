...
class MessageReader:
    ...
    def should_commit(self, isContinuous=None):
        if isContinuous:
            return Utils.current_time_mills() - self.lastCommitTime >= self.commitInterval\
                   or self.finishedOffset - self.lastCommitOffset >= self.commitThreshold
        else:
            return Utils.current_time_mills() - self.lastCommitTime >= self.commitInterval\
                   and self.finishedOffset - self.lastCommitOffset >= self.commitThreshold
    ...
    def process_fetch_exception(self, e=None):
        ...
        self.logger.warn("process unexcepted fetchException:" + e.message)
