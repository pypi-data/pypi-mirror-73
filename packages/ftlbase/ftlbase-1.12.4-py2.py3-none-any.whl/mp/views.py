...
class NotificationView(CSRFExemptMixin, View):
    ...
    def post(self, request, ):  # reference):  #  Não tem mais referência sendo passado
        ...
        # reference = data.get('data', {}).get('id', request.GET.get('id'))
        reference = data.get('id', data).get('data.id', request.GET.get('data.id'))
        logger.info('Got POST notification: %s', data)
        form = forms.NotificationForm({'topic': topic, 'id': reference, })
        return self.process(request, reference, form)
