...
class SVNTreeError(cvs2hg_test.Failure):
...
# TODO: Check to make sure contents and children are mutually exclusive
    ...
    def as_state(self, prefix=None):
        """Return an cvs2hg_test.wc.State instance that is equivalent to this tree."""
        root = self
        if self.path == root_node_name:
            ...
            state = cvs2hg_test.wc.State(wc_dir, { })
            if root.contents or root.props or root.atts:
    ...
    def as_item(self):
        return cvs2hg_test.wc.StateItem(self.contents,
                                    self.props,
...
def get_props(paths):
    ...
    files = {}
    exit_code, output, errput = cvs2hg_test.main.run_hg(1,
                                                     "proplist",
                                                     "--verbose",
                                                     "--xml",
                                                     *paths)
...
def dump_tree_script(n, subtree="", stream=sys.stdout, wc_varname='wc_dir'):
    ...
    The WC_VARNAME is inserted in the cvs2hg_test.wc.State(wc_dir,{}) call
    that is printed out (this is used by factory.py)."""

    stream.write("cvs2hg_test.wc.State(" + wc_varname + ", {")
...
def build_tree_from_checkout(lines, include_skipped=True):
    ...
    return cvs2hg_test.wc.State.from_checkout(lines, include_skipped).old_tree()
...
def build_tree_from_commit(lines):
    ...
    return cvs2hg_test.wc.State.from_commit(lines).old_tree()
...
def build_tree_from_status(lines):
    ...
    return cvs2hg_test.wc.State.from_status(lines).old_tree()
...
def build_tree_from_skipped(lines):
    ...
    return cvs2hg_test.wc.State.from_skipped(lines).old_tree()


def build_tree_from_diff_summarize(lines):
    "Build a tree from output of diff --summarize"

    return cvs2hg_test.wc.State.from_summarize(lines).old_tree()
...
def build_tree_from_wc(wc_path, load_props=0, ignore_svn=1):
    ...
    return cvs2hg_test.wc.State.from_wc(wc_path, load_props, ignore_svn).old_tree()
