...
PUBLIC_VARS = {'WebDriverWait': 30, 'poll_frequency': 0.5}

def cmd2action(b=None, r=None, loc=None, data=None, contains_assert=None, equal_assert=None):
    if r[0][0] == 1:
        if loc not in ('', None):
            if isinstance(b, WebDriver):
                WebDriverWait(b, PUBLIC_VARS['WebDriverWait'], PUBLIC_VARS['poll_frequency'])\
                    .until(lambda b: b.find_element_by_xpath(loc))
