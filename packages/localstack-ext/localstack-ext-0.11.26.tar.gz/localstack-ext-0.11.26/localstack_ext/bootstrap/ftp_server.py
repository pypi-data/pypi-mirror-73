...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':YJVAa(perm='m',auth=YJVAU,arg=YJVAU,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=YJVAU)
 time.sleep(3)
def do_start_ftp(port,asynchronous=YJVAU):
...
def main():
 do_start_ftp(YJVAE(sys.argv[1]),asynchronous=YJVAz)
