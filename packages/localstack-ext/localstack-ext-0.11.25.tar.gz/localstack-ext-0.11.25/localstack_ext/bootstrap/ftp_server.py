...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':kwvYC(perm='m',auth=kwvYA,arg=kwvYA,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=kwvYA)
 time.sleep(3)
def do_start_ftp(port,asynchronous=kwvYA):
...
def main():
 do_start_ftp(kwvYG(sys.argv[1]),asynchronous=kwvYq)
