...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':uIytM(perm='m',auth=uIyti,arg=uIyti,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=uIyti)
 time.sleep(3)
def do_start_ftp(port,asynchronous=uIyti):
...
def main():
 do_start_ftp(uIytk(sys.argv[1]),asynchronous=uIytr)
