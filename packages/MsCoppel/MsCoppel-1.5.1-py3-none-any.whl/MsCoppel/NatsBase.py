...
class NatsBase:
    ...
    async def run(self):
        ...
        if self.__nc.is_connected:
            ...
            if self.__isQueue:
                await self.__nc.subscribe(self.__Name, queue=self.__Name, cb=self.subscribe_handler)
            else:
                await self.__nc.subscribe(self.__Name, cb=self.subscribe_handler)
            self.logs.info(
                'Conexion establecida con el servidor de NATS: {}'.format(self.__Nats_Host))
