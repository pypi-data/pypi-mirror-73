...
class AzureBatch(BackendBase):
    ...
    def wrap_cmd_for_batch(self, user_cmd, node_id, store_data_dir, data_action, data_writable, 
        ...
        super().add_first_cmds(cmds, change_dir=False, args=args)

        # copy bootstrap files from docker volume /usr/src to current dir
        self.append_title(cmds, "unzip bootstrap files")

        # unzip bootstrap files into current dir
        if docker_cmd:
            fn_bootstrap = "/usr/src/" + constants.BOOTSTRAP_ZIP_FN
            self.append_unzip(cmds, fn_bootstrap, ".")
        else:
            fn_bootstrap = constants.BOOTSTRAP_ZIP_FN
            self.append_unzip(cmds, fn_bootstrap, ".")
            self.append(cmds, "rm {}".format(fn_bootstrap))

        self.append(cmds, "ls -lt")

        super().add_setup_cmds(cmds, args=args)
        super().add_xt_setup_cmds(cmds, args, is_windows=False)
    ...
    def prep_node(self, job_id, node_index, run_data_list, num_nodes, use_low_pri, using_hp,  
        ...
        before_path = "before/bootstrap"      # job store
