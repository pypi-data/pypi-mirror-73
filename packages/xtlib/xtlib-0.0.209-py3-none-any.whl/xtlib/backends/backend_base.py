...
class BackendBase(implements(BackendInterface)):
    ...
    def append(self, cmds, cmd, expand=False, log=None, echo=None):
        ...
        if echo:
            ...
            if self.add_time:
                if self.gen_for_windows:
                    # this fancy form of echo allows us to group complex strings in dbl quotes without showing the quotes on output
                    cmds.append('''echo|set /p= "@%date% %time%     ++ {}"'''.format(cmd_text))
                else:
    ...
    def create_blobfuse_commands(self, storage_name, storage_key, sudo_available, mount_requests, install_blobfuse, 
        ...
        for md in mount_requests:
            ...
            self.append(cmds, "ls -l {}".format(mnt_dir))
    ...
    def process_action(self, cmds, action, mnt_path, env_var_name, env_var_name2, container, store_data_dir, is_writable,
        ...
        if action == "mount":
            ...
            setter = "set" if gen_for_windows else "export"
    ...
    def add_xt_setup_cmds(self, cmds, args, is_windows):
        ...
        if self.gen_for_windows:
            if not pp:
                pp = [".", "%PYTHONPATH%"]
            path = ";".join(pp)
            path = file_utils.fix_slashes(path, is_linux=False)
            self.append(cmds, 'set PYTHONPATH={}'.format(path))
        else:
            if not pp:
                pp += [".", "$PYTHONPATH"]
            path = ":".join(pp)
            path = file_utils.fix_slashes(path, is_linux=True)
            self.append(cmds, 'export PYTHONPATH={}'.format(path))
    ...
    def append_dir(self, cmds, path="."):
        if False:  #self.gen_for_windows:
            cmd = "dir /w {}".format(path)
        else:
            cmd = "ls -lt {}".format(path)
    ...
    def add_info_cmds(self, cmds, args):
        ...
        self.add_info(cmds, "node id", node_id)
        self.add_info(cmds, "run name", run_name)
        ...
        if self.gen_for_windows:
            ...
            cmds.append("wmic path win32_VideoController get name  | findstr -V Name > __t__ && set /p xt_tmp= < __t__")
            self.add_info(cmds, "GPU type", "%xt_tmp%")
            cmds.append("python -V 2> __t__ && set /p xt_tmp= < __t__")
            ...
            self.add_info(cmds, "Python", "$(python -V)")
    ...
    def framework_report(self, cmds):
        ...
        if self.gen_for_windows:
            ...
            # the "2>&1" is used to join stderr and stderr here since versions of python use both
            self.add_info(cmds, "Python", "$(python -V 2>&1)")

        # PYTORCH
        target_cmd = '''python -c "import torch; print('PyTorch: \t' + torch.__version__ + ', CUDA available: ' + str(torch.cuda.is_available()))"'''
        self.append_package_test(cmds, "torch", target_cmd, "PyTorch")

        # TENSORFLOW
        target_cmd = '''python -c "import tensorflow as tf; print('Tensorflow: \t' + tf.__version__ + ', CUDA available: ' + str(tf.test.is_gpu_available()))"'''
    ...
    def wrap_user_command(self, user_parts, snapshot_dir, store_data_dir, data_action, data_writable, \
        ...
        sudo_available=True, username=None, use_username=True, install_blobfuse=False, 
        ...
        node_id = "%1%" if gen_for_windows else "$1"
        run_name = "%2%" if gen_for_windows else "$2"
        ...
        self.add_setup_cmds(cmds, args)

        self.add_xt_setup_cmds(cmds, args, gen_for_windows)
        ...
        if not self.gen_for_windows:
            ...
            store_path = "runs/{}/output".format(run_name)   # uses RUN_NAME arg (passed to script)
        ...
        if self.gen_for_windows:
            self.append(cmds, 'set XT_NODE_ID={}'.format(node_id))
        else:
            self.append(cmds, 'export XT_NODE_ID={}'.format(node_id))
    ...
    def create_wrapper_and_inner(self, cmds, user_cmd, args):
        ...
        if docker_cmd:
            ...
            self.append_title(docker_cmds, 'START of XT-level processing', double=True, index=0)
    ...
    def add_setup_cmds(self, cmds, args):
