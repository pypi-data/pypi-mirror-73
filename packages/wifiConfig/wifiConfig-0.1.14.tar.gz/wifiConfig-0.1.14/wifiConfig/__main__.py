...
def main():
    ...
    if argu.command == 'start':
        print(header)
        print("["+bcolor.text("INFO")+"] Loading...")
        args = vars(argu)
        ...
        print("["+bcolor.text("OK")+"] App loaded")
        
        flag=0
        if args["check"] == "True":
            print("["+bcolor.text("INFO")+"] Checking network con")
            if not check_con():
                print("["+bcolor.text("FAILED")+"] You dont have an internet con")
                flag=1
            else:
                print("["+bcolor.text("OK")+"] You have an internet con\n\tIf you want to force the start, please use the arg --check False")              
        else:
            flag=1
        if flag: myWifiConfApp.start()
