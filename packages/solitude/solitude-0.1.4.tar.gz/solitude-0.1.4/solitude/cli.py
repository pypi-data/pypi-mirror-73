...
class SharedCommand(BaseCommand):
    def __init__(self, *args, **kwargs):
        ...
        self.params.insert(0, click.core.Option(param_decls=(
            "-f", "--cmd-files"), multiple=True, required=True, type=str, default=Config().defaults.cmdfiles,
...
def are_jobids_specified(jobids: List):
    ...
    if not are_specified:
        click.echo("This operation requires specification of jobids with -i or --jobids")
...
@click.option("--priority", type=click.Choice(["idle", "low", "high"]), default="low",
              help="Set default priority to run jobs with.")
@click.option("--cmd-files", type=str, multiple=True, default=(),
...
def create(user, workers, priority, cmd_files, ssh_server, ssh_user, ssh_pass, force):
    cfg = Config()
