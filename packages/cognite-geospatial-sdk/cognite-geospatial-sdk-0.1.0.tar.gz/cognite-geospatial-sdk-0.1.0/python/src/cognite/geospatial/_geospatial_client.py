...
class CogniteGeospatialClient:
    ...
    def find_within_completely_async(
        ...
        """The result is identical to Within except when the feature in the input feature layer intersects
        the boundary of the feature in the selecting features layer; then it is not selected.
        """
        geometry = self._create_geometry(id, external_id, geography)
        proj = _create_projection(projection)
        spatial_filter = SpatialFilterObject(
    ...
    def find_within_completely(
        ...
        """The result is identical to Within except when the feature in the input feature layer intersects the
        boundary of the feature in the selecting features layer; then it is not selected.
        """
        ...
        if item is not None:
            for name in attributes:
                ...
                if not _is_primitive(value):
                    _ = yield self.add_spatial_item_data_async(item.id, name, value)
