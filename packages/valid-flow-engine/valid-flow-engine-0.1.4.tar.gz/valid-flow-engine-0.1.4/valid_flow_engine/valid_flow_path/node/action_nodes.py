    ...
    def run_op(self, payload: dict):
        ...
        for arg_def in op_args:
            if isinstance(arg_def, dict):
                ...
                if arg_def.get('payload_element', False):
...
class ActionNode(Node, ABC):
    ...
    def parse_node(func: dict) -> _OpRunner:
        ...
        """
        block_set = func.get('block_set_key')
        args = func.get('args')
        block_key = func.get('block_key')
        block_dict = {
            'block_key': block_key,
            'args': args,
        }
    ...
    @abstractmethod
    def run(self, path: ValidFlowPath, payload: dict):
...
class BooleanNode(ActionNode):
    ...
    def __init__(self, node_def: dict):
        ...
        self.false_targets = node_def.get('false_targets')
        self.true_targets = node_def.get('true_targets')
...
class FunctionNode(ActionNode):
    ...
    def __init__(self, node_def: dict):
        ActionNode.__init__(self, node_def)
        self.output_source_node_ids = node_def.get('targets')
        self.return_key = node_def.get('return_key', None)

    def run(self, path: ValidFlowPath, payload: dict):
        op_res = self.node.run_op(payload=payload)
        if self.return_key is not None:
            payload[self.return_key] = op_res
        results = []
        for output in self.output_source_node_ids:
            ActionNode.run_targets(results, output, payload, path)
        return results, {self.id: op_res}
