# -*- coding: UTF-8 -*-
#   Copyright 2009-2020 Oli Schacher, Fumail Project
#
...
try:
    from dkim import DKIM, sign, Simple, Relaxed, DKIMException, MessageFormatError, ARC
    ...
    def examine(self, suspect):
        ...
        for hdrname in ARC.ARC_HEADERS:
            ...
            if hdrname not in msgrep:
                suspect.set_tag('ARCVerify.skipreason', 'not dkim signed')
...
It is currently recommended to leave both header and body canonicalization as 'relaxed'. Using 'simple' can cause the signature to fail.
    ...
    def examine(self, suspect):
        ...
        try:
            ...
            try:
                valid = d.verify()
                self.logger.debug("%s: DKIM validity %s" % (suspect.id, valid))
            ...
            suspect.write_sa_temp_header(DKIM_SAHEADER, DKIM_VALID if valid else DKIM_INVALID)
        except MessageFormatError as e:
