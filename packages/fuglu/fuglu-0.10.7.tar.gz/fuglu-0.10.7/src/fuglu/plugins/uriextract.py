...
from fuglu.shared import (
    ScannerPlugin, DUNNO, string_to_actioncode,
    apply_template, FileList, AppenderPlugin,
    HAVE_BEAUTIFULSOUP, Suspect
)
...
class URIExtract(ScannerPlugin):
    ...
    def get_decoded_textparts(self, suspect,bcompatible=True, ignore_words_without=(),
                              maxsize=None, maxsize_analyse=None, hrefs=None):
        ...
        for attObj in att_mgr.get_objectlist():
            ...
            if attObj.content_fname_check(contenttype_start="text/") \
                ...
                if attObj.content_fname_check(contenttype_contains="html") \
                    ...
                    decoded_payload = invisible.sub("", decoded_payload)
                    ...
                    if BeautifulSoup:
                        ...
                        if isinstance(hrefs, list):
                            atags = BeautifulSoup.BeautifulSoup(decoded_payload, "lxml").find_all('a')
                            if atags:
                                atags = [atag.get("href") for atag in atags if atag.get("href")]
                            # add to hrefs list
                            hrefs.extend(atags)
