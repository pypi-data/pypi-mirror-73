...
class Suspect(object):
    ...
    def get_client_info(self, config=None):
        ...
        if config is None:
            ...
            clientinfo = self.client_info_from_rcvd(trustedhostsregex, boundarydistance, skiponerror)
        self.clientinfo = clientinfo
        return clientinfo

    def client_info_from_rcvd(self, ignoreregex=None, skip=0, skiponerror=False):
        """returns information about the client that submitted this message.
        (helo,ip,reversedns)
        ...
        for rcvdline in receivedheaders[skip:]:
            ...
            self.logger.debug(f"{self.id} (client_info_from_rcvd) Parsed: helo={helo}, revdns={revdns}, ip={ip}")
