...
class Detector:
    ...
    def __post_init__(self):
        self.fault_ranges = {2800: "detector-low-soft-fault",
...
class ImacModule:
    ...
    async def parameter_block_reader(self, data: set):
    ...
    async def find_missing_starting_data(self):
        ...
        if missing:
            ...
            if not params:
                raise FailedAddressDiscover(f"Failed to read {self.name} parameters")
...
class GasGuard2(ImacModule):
    ...
    starting_params = ["address-flags", "address-analog", "address-power", "address-bypass"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.block_specs = {
            ...
            "warmup-config": ParamMaskBool(0x411, "warmup-config", mask=1 << 8, rshift=8, block=2),
            ...
            "cal-cup-alarm-28-days": ParamMaskBool(0x40f, "cal-cup-alarm-28-days", mask=1 << 4, rshift=4, block=5),
            "linearity-test-alarm-14-days": ParamMaskBool(0x40f, "linearity-test-alarm-14-days", mask=1 << 3, rshift=3,
                                                          block=5),
        ...
        for postbox, code in postboxes.items():
            async with self.postbox_lock:
                try:
                    ...
                    log.error(f"Failed to read {self.current_state['dev-id']}: {postbox}")
        return parameters
