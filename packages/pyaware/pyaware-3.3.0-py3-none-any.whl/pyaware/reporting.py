...
class Report:
    ...
    def mean(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.mean, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def sum(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.sum, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def max(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.max, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def min(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.min, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def latest(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.latest, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def samples(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(
            self._build_filter(Aggregate.samples, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                               not_dev_ids=not_dev_ids))

    def all(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.all, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))

    def median(self, labels=None, not_labels=None, dev_ids=None, not_dev_ids=None):
        self._filters.append(self._build_filter(Aggregate.median, labels=labels, not_labels=not_labels, dev_ids=dev_ids,
                                                not_dev_ids=not_dev_ids))
        ...
        for dev_id, label, data_points in data:
            ...
            if aggs:
                if not dev_id in report_data:
                    report_data[dev_id] = {}
                report_data[dev_id][label] = {agg.__name__: agg(data_points) for agg in aggs}
        return report_data
    ...
    def report(self, data):
        ...
        for dev_id, dev_type, label, data_points in data:
            ...
            if aggs:
                ...
                report_data[dev_id]["data"][label] = {agg.__name__: agg(data_points) for agg in aggs}
