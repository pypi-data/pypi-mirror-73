...
roll_call_params = {"generation_id": ParamMask(0x409, "generation_id", mask=0b11 << 8, rshift=8),
...
@events.enable
class MockImac2Protocol(Imac2Protocol):
    def __init__(self, roll_call, device_id: str, eth_client=None, _async=False):
        super().__init__(MagicMock(), stop_evt=threading.Event(), client_eth=eth_client or MagicMock(), _async=_async,
                         device_id=device_id)
        self.block_mimic = {}
    ...
    def _read_by_serial_number_sync(self, serial_number, generation, block=0) -> AddressMapUint16:
        ...
        if key not in self.block_mimic:
            ...
            self.block_mimic[key] = {k: AddressMapUint16() for k in [None, 0, 1, 2, 3, 4, 5, 6, 7]}
    ...
    async def _roll_call_force_async(self, address) -> [AddressMapUint16]:
        ...
        events.publish("force_roll_call_complete", data=address, timestamp=datetime.utcnow())
        return modules
...
def simulate_gg2(serial_number, generation_id, number: int = 0) -> [AddressMapUint16]:
    ...
    return simulate_roll_call(
        *({"generation_id": generation_id, "serial_number": serial_number, "version": 0, "imac_address": addr,
           "module_type": mod_type} for addr, mod_type in zip(addresses, [61, 62, 63])))
