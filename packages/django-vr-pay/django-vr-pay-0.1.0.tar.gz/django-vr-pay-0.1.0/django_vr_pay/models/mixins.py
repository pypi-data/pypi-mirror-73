...
class APIResponseMixin(models.Model):
    http_status_code = models.IntegerField(
        "HTTP status code", help_text="response status code"
    )
    url = models.URLField("URL", help_text="request url")
    raw_headers = BaseJSONField("Headers", help_text="response header as json")
    raw_content = BaseJSONField("Content", help_text="response content as json")
