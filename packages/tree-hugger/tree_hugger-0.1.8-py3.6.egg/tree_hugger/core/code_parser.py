...
def walk_recursive(cursor: TreeCursor, lines: List, apply):
    n :Node = cursor.node
    if n.children:
        for ch in n.children:
            if ch.type == "subscript":
                cursor.goto_first_child()
                if cursor.node.children:
                    start = False
                    pp = {}
                    key = match_from_span(cursor.node.children[0], lines)
                    pp[key] = []
                    for x in cursor.node.children:
                        if x.type == "[":
                            start = True
                        if start:
                            if x.type == "string":
                                pp[key].append(match_from_span(x, lines))
                        if x.type == "]" and start:
                            start = False
                    # print(pp)
                    apply(pp)
            cursor = ch.walk()
            walk_recursive(cursor, lines, apply)
