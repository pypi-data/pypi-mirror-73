    ...
    def __init__(self, *args, **kwargs):
        ...
        class OperationsItem:
            def __init__(self, operation_item):
                ...
                self.operation_resource = Operation_resources.objects.get(operationitem=operation_item)
                self.resource = Operation_resources.objects.get(operationitem=operation_item).resource
        ...
        with connection.cursor() as cursor:
            ...
            self.resources = [operation_item.resource for operation_item in self.operations_item.stack]
...
class OperationsManager(CommonManager):
    ...
    def delete_production_order(data):
        ...
        opertypes = (
            ...
            settings.OPERS_TYPES_STACK.PRODUCTION_DETAIL_SUM_TASK.id,
        )
        ...
        for parent_launch_id in launch_ids:
            ...
            with connection.cursor() as cursor:
                cursor.execute('''select po.id
                                    from planing_operations as po
                                             join planing_operation_launches as pllch on po.id = pllch.operation_id
                                    where po.opertype_id in %s
                                      and pllch.launch_id = %s''', [(settings.OPERS_TYPES_STACK.PRODUCTION_TASK.id,), parent_launch.id])
                ...
                with managed_progress(
                    ...
                    with transaction.atomic():
                        for row in rows:
                            ...
                            def pre_delete_functtion(operation):
                                operation_id = operation.id

                                operation_executor_cnt = Operation_executor.objects.filter(operation_id=operation_id, props=Operation_executor.props.relevant).count()

                                if operation_executor_cnt > 0:
                                    user = Operation_executor.objects.filter(operation_id=operation_id, props=Operation_executor.props.relevant)[0].executor

                                    operation_item = Operation_item.objects.get(operation_id=operation_id)
                                    operation_resources = Operation_resources.objects.filter(operation_id=operation_id).order_by('id')[0]

                                    message = blinkString(
                                        f'Удаление задания на производство № {operation.num} '
                                        f'от {DateTimeToStr(operation.date, hours=3)}<p/>'
                            ...
                            OperationsManager.delete_recursive(
                                operation=operation,
                                user=user,
                                opertypes=','.join([str(i) for i in opertypes]),
                                pre_delete_function=pre_delete_functtion
                            )
