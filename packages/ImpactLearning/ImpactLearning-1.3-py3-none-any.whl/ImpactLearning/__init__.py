...
######################################################################################
  ...
  #------------------------------------------------------------------
    ...
    def train(self,epochs=2000, lr=0.009, progress_per=100, kfold=None):
        self.epochs = epochs
        self.lr = lr
        self.temp_loss = 0
        self.kfold = kfold
        ...
        if self.kfold is None:
          ...
          self.skf = KFold(n_splits=self.kfold)
          self.val_acc_scores = {
              'train_acc':[],
              'test_acc':[],
              'train_loss':[],
          ...
          fold_count = 0
          for train_index, test_index in self.skf.split(self.Xdata, self.ydata):
              self.X = tf.convert_to_tensor(self.Xdata[train_index], dtype=tf.float32)
              self.y = tf.convert_to_tensor(self.ydata[train_index], dtype=tf.float32)
             
              self.X_test = tf.convert_to_tensor(self.Xdata[test_index], dtype=tf.float32)
              self.y_test = tf.convert_to_tensor(self.ydata[test_index], dtype=tf.float32)
...
######################################################################################
  ...
  #------------------------------------------------------------------
    ...
    def train(self,epochs=2000, lr=0.009, progress_per=100, kfold=None):
        ...
        self.ac_score = 0
        if self.kfold is None:
          self.X = tf.convert_to_tensor(self.Xdata)
          self.y = tf.convert_to_tensor(self.ydata)
          for epoch_count in range(1, self.epochs+1):
                  self.optimizer(self.lr)
          ...
          self.skf = StratifiedKFold(n_splits=self.kfold)
          self.val_acc_scores = {
              'train_acc':[],
              'test_acc':[],
              'train_loss':[],
          ...
          fold_count = 0
          for train_index, test_index in self.skf.split(self.Xdata, self.ydata):
              self.X = tf.convert_to_tensor(self.Xdata[train_index])
              self.y = tf.convert_to_tensor(self.ydata[train_index])
              self.y_true_onehot = tf.one_hot(self.y, depth=self.num_classes)

              self.X_test = tf.convert_to_tensor(self.Xdata[test_index])
              self.y_test = tf.convert_to_tensor(self.ydata[test_index])
              self.t_y_true_onehot = tf.one_hot(self.y_test, depth=self.num_classes)
             


              self.WeightX = tf.Variable(tf.ones( [self.Xdata.shape[1], self.num_classes ]) )
              self.Bias = tf.Variable(tf.zeros([self.num_classes]))
              self.WeightY = tf.Variable(tf.ones([]))
              self.RNI = tf.Variable(tf.ones([]))
              fold_count+=1
              print("Fold {}".format(fold_count))
