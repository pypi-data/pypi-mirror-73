...
def mapmask(arr, itr= 5, kern_rad=3):
    import numpy as np
    '''X2 = np.sort(arr.flatten())
    F2 = np.array(range(len(X2)))/float(len(X2))
    thresh = X2[np.where(F2== 0.99)[0]]
    thresh = thresh.flatten()
    dilatemask = binary_dilation_ccmask((arr > thresh[0]).astype(int))'''
    dilatemask = binary_dilation_ccmask(arr, iter=itr)
    mask = make_soft(dilatemask, kern_rad)
    return mask
