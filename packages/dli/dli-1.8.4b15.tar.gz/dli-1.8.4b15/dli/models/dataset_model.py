...
try:
    import dask.dataframe as dask_dataframe  # noqa
except ImportError:
    dask_dataframe = None
...
class DatasetModel(AttributesDict):
    ...
    def dask_dataframe(
        ...
        .. code-block:: python
            df = dataset.dask_dataframe()

        """
        # XXX todo - decide what arguments we can allow passing in.
        ...
        if self.data_format.lower() == 'parquet':
            return dask_dataframe.read_parquet(
        ...
        if self.data_format.lower() == 'csv':
            return dask_dataframe.read_csv(
                f's3://{self.organisation_short_code}/{self.short_code}' + (
                    '' if self.short_code.endswith('/') else '/'
                ),
                storage_options={
