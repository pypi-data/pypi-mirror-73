...
def get_13f_holdings(filer_id: int, config: WhaleWisdomConfig, start_date: datetime.date=None, end_date: datetime.date=None,
    ...
    if stock_ids != None:
        max_rank = max(df.current_ranking.max(axis=0, skipna=True), df.previous_ranking.max(axis=0, skipna=True)) 

    df.drop(columns=["shares_change"], inplace=True)
    ...
    df["change_in_mv"] = df["current_mv"] - df["previous_mv"]
