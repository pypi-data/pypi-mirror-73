...
class ClientNetService(Service):
    def __init__(
        ...
        self._uuid = tmp_uuid.bytes
        self._uuid_str = str(tmp_uuid)
    ...
    def handle_authentification(self, command):
        ...
        if 'FAILURE' in connection_status:
            self.notify_event('CONNECTION_REFUSED')
    ...
    def main(self, sockets):
        ...
        if self.state == STATE_AUTH:
            if (current_milli_time()-self._connection_start_time) > AUTH_TIMEOUT:
...
class ServerNetService(threading.Thread):
    ...
    def _register_client(self, auth_type, uid, id, password):
    ...
    def run(self):
        ...
        fh = logging.FileHandler('D:/server.log')
        ...
        while not self._exit_event.is_set():
            ...
            if self._command in socks:
                ...
                if isinstance(command, RepAuthCommand):
                    ...
                    auth_status = self._register_client(auth_type,
                                                        auth_origin,
                                                        auth_id,
                                                        auth_pass)
...
class ServerTTL(threading.Thread):
    ...
    def run(self):
        ...
        self.logger.setLevel(logging.INFO)
        # create file handler which logs even debug messages
        fh = logging.FileHandler('D:/ttlserver.log')
        self.logger.addHandler(fh)
