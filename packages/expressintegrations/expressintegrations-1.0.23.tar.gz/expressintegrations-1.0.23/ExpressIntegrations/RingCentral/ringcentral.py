...
class ringcentral:
  ...
  def renew_webhook(self, subscription_id):
    ...
    if not result.ok:
      raise Exception(f"Failed to renew webhook. Result: {result.json()}")
    return result.json()
  ...
  def create_contact(self, contact, dialing_plan=None):
    ...
    if not result.ok:
      raise Exception(f"Failed to create contact. Result: {result.json()}")
    return result.json()
  ...
  def get_contacts(self, starts_with=None, sort_by=None, page=None, per_page=None, phone_number=None):
    ...
    if not result.ok:
      raise Exception(f"Failed to search contacts. Result: {result.json()}")
    return result.json()

  def get_contact(self, contact_id):
    result = self.platform.get(f"/restapi/v1.0/account/~/extension/~/address-book/contact/{contact_id}")
    if not result.ok:
      raise Exception(f"Failed to get contact. Result: {result.json()}")
    return result.json()
  ...
  def update_contact(self, contact_id, contact, dialing_plan=None):
    ...
    if not result.ok:
      raise Exception(f"Failed to update contact. Result: {result.json()}")
    return result.json()

  def delete_contact(self, contact_id):
    result = self.platform.delete(f"/restapi/v1.0/account/~/extension/~/address-book/contact/{contact_id}")
    if not result.ok:
      raise Exception(f"Failed to delete contact. Result: {result.json()}")
    return result.json()
  ...
  def search_company_directory(self, search_string=None, show_federated=True, extension_type=None, order_by=None, page=None, per_page=None):
    ...
    if not result.ok:
      raise Exception(f"Failed to search company directory. Result: {result.json()}")
    return result.json()
