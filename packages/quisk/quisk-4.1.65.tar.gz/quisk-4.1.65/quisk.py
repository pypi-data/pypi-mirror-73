  ...
  def SetSoundDevices(self):
    ...
    for name, is_play, index in sound_cards:
      ...
      if value[0:6] == 'pulse:':
        ...
        continue
      if conf.use_fast_sound and name == "name_of_sound_play":
        alsa_driver = DEV_DRIVER_SOUNDIO
      else:
        alsa_driver = DEV_DRIVER_ALSA
      if value[0:5] == 'alsa:':
        ...
        if not found_desc:
          found_desc = value
          found_dev = value[5:]
        QS.set_sound_name(index, is_play, alsa_driver, found_desc, found_dev)
        if conf.pulse_audio_verbose_output:
          print (name, "Alsa driver %d" % alsa_driver, found_desc, ';', found_dev)
        continue
      ...
      if not found_desc:
        a_dev = value
        a_desc = value
      QS.set_sound_name(index, is_play, alsa_driver, a_desc, a_dev)
      if conf.pulse_audio_verbose_output:
        print (name, "Alsa driver %d" % alsa_driver, a_desc, ';', a_dev)
