...
def plot_colorbar(absmax, cmap='RdYlGn'):
    ...
    ax.imshow(gradient, extent=[-absmax,absmax,0,1], aspect='auto', cmap=plt.get_cmap(cmap))
...
def _get_percent_color(value, absmax):
    """ Related to a bar chart, returns the percent coloring in either direction assuming 50% is the middle """
    return 50 + value/absmax*50

def _get_gradient_string(value):
    if value > 50:
        value = max(53, value) # Try to make sure you can always see the sliver of contribution, even if it's small.
...
def _handle_ensemble(ensembler, dataframe, bg_data, target):
    importances = pd.DataFrame([], columns=dataframe.columns)
    bg_importances = pd.DataFrame([], columns=bg_data.columns) # Oof

    # pd.concat will add multiple entries for the same index value
    for graph in ensembler.graphs:
        explainer = KernelShap(graph, bg_data, target=target)
    ...
    return importances.fillna(0).groupby(by=importances.index).agg('mean').set_index('index', drop=True), bg_importances.fillna(0).groupby(by=bg_importances.index).agg('mean').set_index('index', drop=True)


def paint_df_importance_heatmap(graph, dataframe, bg_data = None, kind='bar', target=None, legend=True, cmap='RdYlGn'):
    """ Paints a Pandas DataFrame according to feature importance in the provided model.
        Parameters:
            ...
            [target]: The feature in your dataframe that is your target variable. None assumes the last element is target
    ...
    if 'FeynEnsemble' in str(type(graph)):
        if bg_data is None:
            bg_data = dataframe
        importance_values, bg_importance = _handle_ensemble(graph, dataframe, bg_data, target)
    elif bg_data is not None:
        shap_explainer = KernelShap(graph, bg_data, target=target)
        bg_importance = shap_explainer.SHAP(bg_data, format = 'pandas')
        importance_values = shap_explainer.SHAP(dataframe, format = 'pandas')
    else:
        shap_explainer = KernelShap(graph, dataframe, target=target)
        importance_values = shap_explainer.SHAP(dataframe, format = 'pandas')

    # TODO: Clean up the if/else statements a bit to reduce duplication. Maybe always set bg_data to dataframe if none and accept double work?

    get_absmax = lambda x: max(x.values.max(), np.abs(x.values.min()))
    if bg_data is not None:
        absmax =  get_absmax(bg_importance)
    else:
        absmax = get_absmax(importance_values)

    if kind not in ['bar', 'fill']:
        raise Exception(f"Kind {kind} not recognized. Must be either 'bar' or 'fill'")
    if kind == 'bar':
        painted_df = dataframe.style.apply(_calculating_col_gradient_bar, importance_values = importance_values, absmax = absmax)
    elif kind == 'fill':
        painted_df = dataframe.style.apply(_calculating_col_gradient, importance_values = importance_values, absmax = absmax, cmap = cmap)
        if legend:
            plot_colorbar(absmax, cmap=cmap)
