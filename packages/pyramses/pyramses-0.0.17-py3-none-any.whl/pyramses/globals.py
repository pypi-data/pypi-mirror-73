...
def setLibDir(dirname):
    """Set path to a custom DLL ramses file.
    ...
    try:
        if os.path.exists(dirname) and os.path.isdir(dirname):
            ...
            raise RAMSESError('RAMSES: The path %s does not exist or it is not a directory.' % (dirname))
    except OSError as e:
        raise RAMSESError('RAMSES: The path %s gave an error: ' + e % (dirname))
