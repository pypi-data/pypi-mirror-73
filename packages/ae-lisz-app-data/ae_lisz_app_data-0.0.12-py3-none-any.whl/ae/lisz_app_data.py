...
class LiszDataMixin(ABC):
    ...
    def change_sub_node_sel(self, node: LiszNode, set_sel_to: bool):
        """ change the selection of all the sub-leafs of the passed node to the specified value.

        :param node:            node of which to change the selection of the sub-item leafs.
        :param set_sel_to:      True will only toggle the unselected sub-item leafs, False only the selected ones.
        """
        for item in node:
    ...
    def on_item_sel_change(self, item_id: str, event_kwargs: dict) -> bool:
        """ toggle, set or reset in the current node the selection of a leaf item or of the sub-leafs of a node item.
    ...
    def sub_item_ids(self, item_id: str, leafs_only: bool, hide_sel_val: Optional[bool] = None,
        ...
        :param leafs_only:      True if only include the leaf item ids of the sub-nodes of the node item.
        ...
        if not leafs_only and (hide_sel_val is None or bool(item.get('sel')) != hide_sel_val):
        ...
        if node is not None:
            for item in node:
                if item.get('node'):
                    self.sub_item_ids(item['id'], leafs_only, hide_sel_val=hide_sel_val, node=node, sub_ids=sub_ids)
