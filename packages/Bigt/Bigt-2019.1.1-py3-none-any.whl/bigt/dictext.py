    ...
    def diff(self, this, other, regex=False, skip=[], path=''):
        ...
        for key, value in this.items():
            ...
            r, n, c = comparators[type(value)]. \
                diff(value, other[key], regex, skip, current_path) \
...
class _list(_default):
    ...
    def diff(self, this_list, other_list, regex=False, skip=[], path=''):
        ...
        for this in this_list:
            ...
            filtered = dict(filterSkip(this.items())) \
                if isinstance(this, dict) else this
            for other in other_list:
                r, n, c = comparators[type(this)] \
                    .diff(filtered, other, regex, skip, path)
                if min_not_present is None:
                    min_not_present = n
                if min_changed is None:
                    min_changed = c
                if self._coefficient(min_not_present, min_changed) \
                        > self._coefficient(n, c):
