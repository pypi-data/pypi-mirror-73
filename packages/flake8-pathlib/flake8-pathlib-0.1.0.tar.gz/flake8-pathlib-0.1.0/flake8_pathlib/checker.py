...
class PathlibVisitor(ast.NodeVisitor):
    ...
    def check_for_call_errors(self, node: ast.AST, name: str) -> None:
        try:
            ...
            self.errors.append(partial_error(lineno=node.lineno, col=node.col_offset))
        except KeyError:
            pass
...
class PathlibChecker:
    ...
    tree: ast.AST
    filename: str = "(none)"
    visitor: PathlibVisitor = dataclasses.field(init=False, default=PathlibVisitor)
...
class Error:
    ...
    def as_flake8_tuple(self) -> typing.Tuple[int, int, str, type]:
        return (
            self.lineno,
            self.col,
