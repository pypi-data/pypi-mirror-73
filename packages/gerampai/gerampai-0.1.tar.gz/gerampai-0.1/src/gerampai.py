    ...
    def extract_feed(self, user_id, retry=0, **kwargs):
        try:
            ...
            if retry > self.max_retry:
                ...
                self.extract_feed(user_id, retry=retry, **kwargs)
...
def write(results, username):
    ...
    with tempfile.NamedTemporaryFile(mode='w+t', delete=False, encoding='utf-8', newline='') as temp_results:
        ...
        writer.writerow(['id', 'is_video', 'comment_disabled', 'taken_at_timestamp', 'owner_id', 'owner_username',
                         'viewer_has_liked', 'viewer_has_saved', 'viewer_has_saved_to_collection',
    ...
    print("Save results to {}.csv".format(username))
...
def extract_client(client, args):
    ...
    while has_next:
        ...
        print('Extracting {}/{} ({:,.2f}%)'.format(total, total_media, total/total_media*100), end='\r')
...
def main():
    ...
    parser.add_argument('username', type=str, help='Username instagram tanpa @')
    parser.add_argument('--delay', type=int, default=60, help="Default delay jika terjadi error to many requests")
    parser.add_argument('--page-size', type=int, default=12, help="Jumlah post per halaman")
