...
def basic_run_remap(datum, window, ij, g_args):
    print(window)
    data = datum[0]
    ...
    for i, j in np.ndindex(data.shape[1:]):
        ...
        try:
            ...
            out[0, i, j] = tendencia_utils.trend_remap(trend.slope, trend.p)

    return out.astype(np.uint8)


def run_trend(in_file, out_file, remap, selector):
    ...
    with rasterio.open(in_file) as src:
        ...
        if remap:
            options.update(count=1, dtype=rasterio.uint8, nodata=0)
    ...
    func = basic_run_remap if remap else basic_run
    # run it
    with riomucho.RioMucho([in_file], out_file, func,
                           windows=windows,
