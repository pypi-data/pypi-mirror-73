...
JAVA_PARAMS = {
    ...
    'log_level': os.getenv('MINECRAFT_LOG_LEVEL', 'warning'),
    'log4j_xml': os.getenv(
        'MINECRAFT_LOG4J_CONF',
        os.path.join(os.path.dirname(os.path.realpath(__file__)), 'extra_files/log4j2.xml')
    )
}
...
class JavaRunner:
    ...
    def start_jvm(self):
        ...
        """
        log_opts = '-Dlog4j.configurationFile=' + JAVA_PARAMS.get('log4j_xml')

        if not os.getenv('MINECRAFT_LOG_LEVEL'):
            # Ensures that the log level is set for log4j to parse.
            os.putenv('MINECRAFT_LOG_LEVEL', JAVA_PARAMS.get('log_level'))
