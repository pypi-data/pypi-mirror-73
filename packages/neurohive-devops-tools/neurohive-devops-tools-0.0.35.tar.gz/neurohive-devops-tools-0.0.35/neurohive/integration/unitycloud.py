...
class UnityCloud:
    ...
    def upload_ios_credentials(self, bundle_id, cert_file, prov_file, cert_secret):
        ...
        if isinstance(cert_file, str):
            stream_cert = open(cert_file, 'rb')
        else:
            stream_cert = cert_file
        if isinstance(prov_file, str):
            stream_prov = open(prov_file, 'rb')
        else:
            stream_prov = prov_file
        files = {
            'fileCertificate': stream_cert,
            'fileProvisioningProfile': stream_prov,
        }
        data = {
            "label": label,
