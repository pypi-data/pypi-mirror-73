...
class AppCenter:
    ...
    def get_download_link(self, app_name, build_ver):
        app_id = self._get_appid(app_name)
        builds = self._get_builds_for_app(app_id)
        filtered = list(filter(lambda x: x['version'] == build_ver, builds))
        if not filtered:
            raise AppCenterWrapperException("Cant find build for version")
        release_info = self._get_release_info(app_id, filtered[0]['id'])
