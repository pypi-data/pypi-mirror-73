...
class TimeSeriesDataset(AbstractDNNTimeSeriesDataset):
    ...
    def get_last_lookback_items(self) -> pd.DataFrame:
        ...
        if not self._training_data:
            raise ConfigException("save data is not enabled", has_pii=False)
        if self.lookback is None:
            raise ConfigException("lookback not set", has_pii=False)
        X_df, y_df, settings = self._training_data
        if settings is None:
            raise ConfigException("settings cannot be empty for time series, need time_column", has_pii=False)
        X_df = X_df.copy()
        y_df = y_df.copy()
