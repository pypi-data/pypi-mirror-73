...
class Pinterest:
    ...
    def unfollow(self, user_name: str) -> bool:
        try:
            self.browser.get(UrlCreator.user_url(user_name))
            rand.sleep(1.5, 2)
            user_container = self.browser.find_by('div', attributes={'data-test-id':'user-unfollow-button'})

            if user_container is not None:
                user_element = self.browser.find_by('button', attributes={'class':'RCK Hsu USg Vxj aZc Zr3 hA- GmH adn a_A gpV hNT iyn BG7 NTm KhY'})
                user_element.click()
