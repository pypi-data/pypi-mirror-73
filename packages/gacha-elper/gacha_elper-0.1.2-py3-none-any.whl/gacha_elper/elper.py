...
class Elper:
    ...
    @classmethod
    def wait_until_find(self, template, sim_from=SIMILARITY_VALUE, sim_to=SIMILARITY_VALUE, coord_from=None, coord_to=None, interval=0):
        while not self.find(template, sim_from=sim_from, sim_to=sim_to, coord_from=coord_from, coord_to=coord_to):
            self.wait(interval)
