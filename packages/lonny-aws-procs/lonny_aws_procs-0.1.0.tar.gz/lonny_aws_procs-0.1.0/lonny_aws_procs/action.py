...
def _get_services(*, cluster):
    for arn in _get_service_arns(cluster):
...
def _register_task_definition(*, proc_set, proc):
    return client_ecs.register_task_definition(
        family = proc.name,
        taskRoleArn = proc.role,
        executioNRoleArn = proc.role,
        networkMode = "awsvpc",
        ...
        containerDefinitions = [dict(
            ...
            secrets = list() if proc.secret is None else [dict(
                name = SECRET_ENV,
                valueFrom = proc_set.secret
            )],
            image = proc_set.image,
