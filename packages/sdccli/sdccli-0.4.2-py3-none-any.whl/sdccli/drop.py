...
def get_full_user_created_macros(secure: SdSecureClient):
    ...
    full_macros = []
    for macro in macros:
        tasks.append(asyncio.get_event_loop().run_in_executor(None, secure.get_falco_macro_id, macro['id']))

    for ok, res in asyncio.get_event_loop().run_until_complete(asyncio.gather(*tasks)):
        if not ok:
            errors.append(res)
        else:
