...
SpeechUrlTemplate = "https://voice.reverso.net/RestPronunciation.svc/v1/output=json/GetVoiceStream/voiceName={}" \
...
class ReversoTTS(object):
    ...
    def convert_text(self, voice, pitch, bitrate, msg):
        self._voice = self.get_voice(voice)
        self._pitch = self.get_pitch(pitch)
        self._bitrate = self.get_bitrate(bitrate)
        try:
            url = SpeechUrlTemplate.format(self._voice, self.text_to_bash64(msg), self._pitch, self._bitrate)
