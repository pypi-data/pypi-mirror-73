...
class StoriesHandler(AbstractHandler):
            ...
            for line in stories:
                if line.startswith("## "):
                    ...
                    if find(
                        ...
                        and isinstance(n, IntentStoryNode),
                        maxlevel=2,
                    ):
                        current_intent = find(
                            current_response,
                        ...
                        nlu_node = find(
                            self.nlu.tree, lambda n: n.name == intent_heading
                        )
                    ...
                    if find(
                        ...
                        and isinstance(n, ResponseStoryNode),
                        maxlevel=2,
                    ):
                        ...
                        resp_node = find(
                            self.resp.tree, lambda n: n.name == response_heading
                        )
    ...
    def export_to_pysg_tree(self):
        def insert_branch(branch, parent):
            ...
            branch_type = (
                TYPE_INTENT if isinstance(branch, IntentStoryNode) else TYPE_RESPONSE
            )
            icon = QUESTION_ICON if branch_type == TYPE_INTENT else ANSWER_ICON
            tree_data.Insert(
                parent=parent,
                key=branch.id,
                text=branch.item.name,
                values=[branch_type, values],
                icon=icon,
            )
    ...
    def export_data(self):
        ...
        for leaf in self.tree.leaves:
            if not leaf.siblings or leaf == leaf.parent.children[-1]:
