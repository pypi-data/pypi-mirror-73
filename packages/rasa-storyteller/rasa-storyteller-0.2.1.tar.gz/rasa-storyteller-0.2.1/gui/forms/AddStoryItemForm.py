...
class AddStoryItemForm(object):
    ...
    def layout(self, available_values):
        layout = [
            [
                sg.Text(self.INPUT_LABEL),
                sg.Combo(
                    size=(50, 30),
                    key=self.INPUT_KEY_NAME,
                    font="Any 12",
                    values=tuple(available_values),
                ),
            ],
            [
    ...
    def process(self):
        ...
        while True:
            ...
            if button in DEFAULT_FORM_SUBMIT_ACTIONS:
                new_item = self.stories.add_item(
                    parent_object_id=self.parent_object_key,
                    text=values[self.INPUT_KEY_NAME],
                )
