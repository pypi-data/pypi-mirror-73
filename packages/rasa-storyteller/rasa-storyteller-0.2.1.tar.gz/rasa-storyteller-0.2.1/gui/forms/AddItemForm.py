...
class AddItemForm(object):
    ...
    THERE_ARE_DUPLICATES_IN_EXAMPLES = (
        "There are duplicates in examples. Examples should be unique."
    )
    ...
    def process(self):
        ...
        while True:
            ...
            children_texts = [
                value
                for value in [
                    v for k, v in values.items() if k != self.INPUT_KEY_NAME and v != ""
                ]
            ]
            if button in DEFAULT_FORM_SUBMIT_ACTIONS:
                error = self.validate(new_item, children_texts)
                if not error:
                    self.handler.add_node_with_kids(
                        values[self.INPUT_KEY_NAME], *children_texts
                    )
