...
class PBS(object):   # The script file generator class
    ...
    def write(self):
        ...
        job_cmds = '\n'.join(self.cmds)
        job_cmds = job_cmds.replace('$SCREEN','$(($$ + 99))')
        job_cmds = job_cmds.replace('module load MATLAB/2017a','PATH="/usr/local/matlab/2017a/bin":${PATH}')
        job_cmds = '\n'.join([job_cmd for job_cmd in job_cmds.split('\n') if not job_cmd.strip().startswith('module')])
        job_cmds = '\n'.join([job_cmd for job_cmd in job_cmds.split('\n') if not job_cmd.strip().startswith('--LD_PRELOAD')])
        job_cmds = job_cmds.replace('\\','\\\\')
        job_cmds = job_cmds.replace("'",'"')
        job_cmds = job_cmds.replace('"','\\"')
        job_cmds = job_cmds.replace('`','\`')
        job_cmds = job_cmds.replace('/gpfs22','')
        job_cmds = job_cmds.replace('/gpfs23','')
        job_cmds = job_cmds.replace('$','\$')
