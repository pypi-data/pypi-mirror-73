        ...
        for field in image_set_fields:
            ...
            if image_set:
                        ...
                        if version == image_set.base_version:
                            for v in asset.variations.keys():
                                if v == image_set.base_version \
                                        and image_set.base_transforms.get(v):

                                    images_to_regenerate[version] = asset
                                    break

                                if v not in image_set.images \
                                        and image_set.base_transforms.get(v):
                            ...
                            if image_set.base_transforms.get(version):
                                images_to_regenerate[version] = asset
                    ...
                    if variations:
                        ...
                        for k, v in variations.items():
                            ...
                            if k in image_set.base_transforms:
                                if version != image_set.base_version \
                                        or k not in image_set.images \
                                        or k == image_set.base_version:
                            ...
                            variations[k] = base_transforms + variations[k]
                ...
                if document.get(field):
                    ...
                    # associated with it.
                    for image in document[field].images:
