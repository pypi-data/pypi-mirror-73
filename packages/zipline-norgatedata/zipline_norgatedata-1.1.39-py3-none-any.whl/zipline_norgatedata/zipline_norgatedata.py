    ...
    with maybe_show_progress(symbols, show_progress,
        ...
        for sid, symbol in enumerate(it):
            ...
            if notice_date is None:
                ...
                ac_date =  df.index[df.index.searchsorted(pd.Timestamp(expiration_date)) - 2] # 2 days prior to Expiration
                notice_date = ac_date
            else:
                ...
                ac_date = df.index[df.index.searchsorted(pd.Timestamp(notice_date)) - 2] # 2 days prior to FND
            all_dates = sessions.snap('D')
            valid_dates = all_dates.slice_indexer(start=start_date,end=end_date)
