...
# These are all parameters that are tied to the particular model self.architecture
...
class ImageClassifierRetrainer:
    def __init__(self, image_dir_name, model_dir_name, retrainer_name):
        self.image_dir_name = image_dir_name
        self.model_dir_name = model_dir_name
        self.retrainer_name = retrainer_name

        self.architecture = "mobilenet_1.0_224"
        self.bottleneck_path_2_bottleneck_values = {}
        
        self.image_dir = FileUtils.get_image_directory_path(self.image_dir_name)
        self.output_labels = os.path.join(BASE_DIR, "tensorflow_data", self.model_dir_name, "output_labels.txt")
        self.summaries_dir = os.path.join(BASE_DIR, "tensorflow_data", self.model_dir_name, "retrain_logs")
        self.output_graph = os.path.join(BASE_DIR, "tensorflow_data", self.model_dir_name, "output_graph.pb")
        self.intermediate_output_graphs_dir = os.path.join(BASE_DIR, "tensorflow_data", self.model_dir_name, "intermediate_graph")
        self.bottleneck_dir = os.path.join(BASE_DIR, "tensorflow_data", self.model_dir_name, "bottleneck")
        return
    ...
    def create_image_lists(self, image_dir, testing_percentage, validation_percentage):
        ...
        results = db.select_all(
            "SELECT DISTINCT(CONTROL_KEY) FROM (SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY IS NOT NULL)",
            {})
    ...
    def get_bottleneck_path(self, image_lists, label_name, index, bottleneck_dir,
        ...
        Args:
            ...
            category: Name string of set to pull images from - training, testing, or
            validation.
            self.architecture: The name of the model self.architecture.
        Returns:
            File system path string to an image that meets the requested parameters.
        """
        return self.get_image_path(image_lists, label_name, index, bottleneck_dir, category) + '_' + self.architecture + '.txt'

    def create_model_graph(self, model_info):
        """"Creates a graph from saved GraphDef file and returns a Graph object.
        Args:
            model_info: Dictionary containing information about the model self.architecture.
        Returns:
    ...
    def run_bottleneck_on_image(self, sess, image_data, image_data_tensor, decoded_image_tensor, resized_input_tensor,
                                bottleneck_tensor):
    ...
    def create_bottleneck_file(self, bottleneck_path, image_lists, label_name, index, image_dir, category, sess,
                               jpeg_data_tensor,
    ...
    def get_random_cached_bottlenecks(self, sess, image_lists, how_many, category, bottleneck_dir,
        ...
        if how_many >= 0:
            ...
            for unused_i in range(how_many):
                ...
                image_name = self.get_image_path(image_lists, label_name, image_index, image_dir, category)
    ...
    def get_random_distorted_bottlenecks(self, sess, image_lists, how_many, category, image_dir, input_jpeg_tensor,
        ...
        for unused_i in range(how_many):
            ...
            label_name = list(image_lists.keys())[label_index]
            image_index = random.randrange(MAX_NUM_IMAGES_PER_CLASS + 1)
            image_path = self.get_image_path(image_lists, label_name, image_index, image_dir, category)
            if not gfile.Exists(image_path):
                tf.logging.fatal('File does not exist %s', image_path)
    ...
    def add_final_training_ops(self, class_count, final_tensor_name, bottleneck_tensor, bottleneck_tensor_size):
    ...
    def create_model_info(self):
        """Given the name of a model self.architecture, returns information about it.
        There are different base image recognition pretrained models that can be
        retrained using transfer learning, and this function translates from the name
        of a model to the attributes that are needed to download and train with it.
        Args:
            self.architecture: Name of a model self.architecture.
        Returns:
            Dictionary of information about the model, or None if the name isn't
            recognized
        Raises:
            ValueError: If self.architecture name is unknown.
        """
        ...
        if self.architecture == 'inception_v3':
            ...
            input_mean = 128
            input_std = 128
        elif self.architecture.startswith('mobilenet_'):
            parts = self.architecture.split('_')
            if len(parts) != 3 and len(parts) != 4:
                tf.logging.error("Couldn't understand self.architecture name '%s'",
                                 self.architecture)
                return None
    ...
    but found '%s' for self.architecture '%s'""",
                    version_string)
                return None
            size_string = parts[2]
    ...
    but found '%s' for self.architecture '%s'""",
            ...
            if len(parts) == 3:
                ...
                if parts[3] != 'quantized':
                    tf.logging.error(
                        "Couldn't understand self.architecture suffix '%s' for '%s'", parts[3],
                        self.architecture)
                    return None
                is_quantized = True
            ...
            tf.logging.error("Couldn't understand self.architecture name '%s'")
            raise ValueError('Unknown self.architecture')
    ...
    def start_training(self, username):
        db = Database()
        ...
        try:
            model_size_string = self.get_graph_sizes()[int(model_size)]['value']
            image_size_string = self.get_image_sizes()[int(image_size)]['value']

            self.architecture = "mobilenet_" + model_size_string + "_" + image_size_string
        except:
            Logger.log("error reading data, using default self.architecture: " + str(self.architecture))
        ...
        # Gather information about the model self.architecture we'll be using.
        model_info = self.create_model_info()
        if not model_info:
            tf.logging.error('Did not recognize self.architecture flag')
            return -1
        ...
        image_lists = self.create_image_lists(self.image_dir, testing_percentage,
        ...
        NotificationManager().send_notification_to_users(username, self.retrainer_name,
                                                         "Das Training '"+self.retrainer_name+"' ist abgeschlossen.")
