...
* [`#161 <https://github.com/mwarkentin/django-watchman/pull/161>`_] Cache imported check modules instead of calling ``import_string`` every time the checks are run

  * The original code was taking ~200ms for each request to ``/watchman/``. With caching all requests after the first took ~7ms.
