...
import ncbi.datasets.v1alpha1.dataset_catalog_pb2 as dataset_catalog_pb2
...
class Assembly():
    ...
    def assembly_report(self):
        ...
        '''
        for assembly_report_file in self._files_of_type(dataset_catalog_pb2.File.ASSEMBLY_REPORT):
    ...
    def fastas(self):
        for fasta_file in self._files_of_type(dataset_catalog_pb2.File.FASTA):
            with open(os.path.join(self._bdbag_dir, 'data', fasta_file), "rt") as handle:
                for record in FastaIterator(handle):
                    yield record

    def gff3s(self, filter=None):
        for gff3_file in self._files_of_type(dataset_catalog_pb2.File.GFF3):
...
class BioDataset():
    ...
    def __init__(self, bdbag_dir):
        ...
        self._catalog = JsonPbParse(raw_data, dataset_catalog_pb2.Catalog())
