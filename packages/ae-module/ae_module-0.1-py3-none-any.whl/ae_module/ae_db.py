...
from ae_db_constant import DB_TRADING_DATA_BASE_NAME, KWDB_STOCK_MASTER, NAVER_DAILY, DB_DAILY_DATA_BASE_NAME, \
    DB_MINUTE_DATA_BASE_NAME, DB_TELEGRAM_DATA_BASE_NAME, DB_INVESTOR_DATA_BASE_NAME, KWDB_INVESTOR_PROCESSED, \
    KWDB_TEST, DB_ACCOUNT_DATA_BASE_NAME, DB_CALC_CORREL_DATA_BASE_NAME
from ae_logger import ae_log
from ae_telegram_constant import TELEGRAM_CHANNEL_ID_증시의신, TELEGRAM_CHANNEL_ID_공시정보, TELEGRAM_CHANNEL_ID_주식_공시알리미, TELEGRAM_CHANNEL_ID_아이투자_텔레그램, TELEGRAM_TEST_CHANNEL_NAME
...
class ae_DataBase:
    ...
    def get_db_trading_connection(self, cursor_type=pymysql.cursors.DictCursor):
        if None == self.db_trading_connection:
            self.db_trading_connection = sql.connect(host=self.DB_ADDRESS, database=DB_TRADING_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_trading_con(self):
        if None == self.db_trading_con:
            self.db_trading_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                                .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_TRADING_DATA_BASE_NAME),
                                                encoding='utf8',
                                                echo=False)
    ...
    def get_db_correl_connection(self, cursor_type=pymysql.cursors.DictCursor):
        if None == self.db_calc_correl_connection:
            self.db_calc_correl_connection = sql.connect(host=self.DB_ADDRESS, database=DB_CALC_CORREL_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_correl_con(self):
        if None == self.db_calc_correl_con:
            self.db_calc_correl_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                                .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_CALC_CORREL_DATA_BASE_NAME),
                                                encoding='utf8',
                                                echo=False)
    ...
    def get_db_daily_naver_con(self):
        if None == self.db_daily_naver_con:
            self.db_daily_naver_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                                    .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, NAVER_DAILY),
                                                    encoding='utf8',
                                                    echo=False)
    ...
    def get_db_account_connection(self, cursor_type = pymysql.cursors.DictCursor):
        if None == self.db_account_connection:
            self.db_account_connection = sql.connect(host=self.DB_ADDRESS, database=DB_ACCOUNT_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
        return self.db_account_connection.cursor(cursor_type)

    def get_db_daily_connection(self):
        if None == self.db_daily_connection:
            self.db_daily_connection = sql.connect(host=self.DB_ADDRESS, database=DB_DAILY_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_daily_con(self):
        if None == self.db_daily_con:
            self.db_daily_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                              .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_DAILY_DATA_BASE_NAME),
                                              encoding='utf8',
                                              echo=False)
    ...
    def get_db_minute_connection(self, cursor_type=pymysql.cursors.DictCursor):
        if None == self.db_minute_connection:
            self.db_minute_connection = sql.connect(host=self.DB_ADDRESS, database=DB_MINUTE_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_minute_con(self):
        if None == self.db_minute_con:
            self.db_minute_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                               .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_MINUTE_DATA_BASE_NAME),
                                               encoding='utf8',
                                               echo=False)
    ...
    def get_db_telegram_connection(self, cursor_type=pymysql.cursors.DictCursor):
        if None == self.db_telegram_connection:
            self.db_telegram_connection = sql.connect(host=self.DB_ADDRESS, database=DB_TELEGRAM_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_telegram_con(self):
        if None == self.db_telegram_con:
            self.db_telegram_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                                 .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_TELEGRAM_DATA_BASE_NAME),
                                                 encoding='utf8',
                                                 echo=False)
    ...
    def get_db_investor_connection(self, cursor_type=pymysql.cursors.DictCursor):
        if None == self.db_investor_connection:
            self.db_investor_connection = sql.connect(host=self.DB_ADDRESS, database=DB_INVESTOR_DATA_BASE_NAME, port=self.DB_PORT, user=self.DB_ID, password=self.DB_PWD, charset='utf8')
    ...
    def get_db_investor_con(self):
        if None == self.db_investor_con:
            self.db_investor_con = create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'
                                                 .format(self.DB_ID, self.DB_PWD, self.DB_ADDRESS, self.DB_PORT, DB_INVESTOR_DATA_BASE_NAME),
                                                 encoding='utf8',
                                                 echo=False)
    ...
    def get_all_tables(self, server="aws"):
        ...
        table_names = []
        try:
            dbcur.execute("SELECT * FROM information_schema.tables WHERE TABLE_SCHEMA = '{}'".format(DB_TRADING_DATA_BASE_NAME))
    ...
    def check_table_exist(self, tablename: str, db_name: str = None):
        if db_name == None:
            db_name = DB_TRADING_DATA_BASE_NAME
        result = False
        if db_name == DB_TRADING_DATA_BASE_NAME:
            dbcur = self.get_db_trading_connection()
        elif db_name == DB_ACCOUNT_DATA_BASE_NAME:
            dbcur = self.get_db_account_connection()
        elif db_name == DB_TELEGRAM_DATA_BASE_NAME:
            dbcur = self.get_db_telegram_connection()
        elif db_name == DB_DAILY_DATA_BASE_NAME:
            dbcur = self.get_db_telegram_connection()
        elif db_name == DB_MINUTE_DATA_BASE_NAME:
            dbcur = self.get_db_telegram_connection()
        elif db_name == DB_INVESTOR_DATA_BASE_NAME:
    ...
    def get_last_telegram_data(self, channel_id):
        ...
        if channel_id == TELEGRAM_CHANNEL_ID_증시의신:
            ...
            table_name = TELEGRAM_TEST_CHANNEL_NAME
    ...
    def update_telegram_data(self, df, tele_channel_id):
        try:
            ...
            exist_table = self.check_table_exist(table_name, DB_TELEGRAM_DATA_BASE_NAME)
