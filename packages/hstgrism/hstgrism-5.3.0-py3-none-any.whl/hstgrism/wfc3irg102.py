...
class Wfc3IrG102:
    ...
    ##########
    ##########
    def compute_flatfield(self,tracefile=None):
        flatfield = self.data['flatfile']
        rootname = self.data['ROOTNAME']
        if tracefile is None:
            tracefile = '{0}_trace.csv'.format(rootname)
        obj = FlatField(flatfile=flatfile,tracefile=tracefile)
        obj.compute()
        self.flatfield = obj
    ##########
    ##########
