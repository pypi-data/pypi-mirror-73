...
class _Allocation():
    ...
    def test(self,
        ...
        P_vf__vi=kwargs.get('P_vf__vi', None)
        ...
        if cores > 1:
            ...
            sub_N_vi_vf_z = []
            sub_N = int(N/cores)+1
            sub_N_vi_vf_z = pool.starmap(self._call_allocation_N_times, [(case, calibration, sub_N, monopixel_patches, cols, i, kwargs) for i in range(cores)])
            pool.close()
        else:
    ...
    def _call_allocation_N_times(self, case, calibration, N, monopixel_patches, cols, core, dict_args):
