...
class Render:
    ...
    def run(self, extension):
        ...
        args = [
            ...
            "-continueOnError",
            "-batchRender",
            f'-outputName:{str(self.dest.workdir / f"0.{extension}")}',
        ]
        if self.texture:
        ...
        for file in follow.iterate_files(process.stdout):
            ...
            self.client.upload_file(
                self.dest.task_id / 'result' / view / file.name,
                file
            )
...
def run(
        ...
        dest_key: str,
        texture: str = None,
        extension: str = Render.DEFAULT_EXTENSION
):
    ...
    try:
        rc = record_time(render.run)(extension)
        if rc > 0:
