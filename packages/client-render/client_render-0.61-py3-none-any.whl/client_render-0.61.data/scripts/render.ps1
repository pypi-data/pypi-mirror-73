...
$Process = Start-Process -FilePath "python" -ArgumentList "-m render.watch Corona --delay=$WatchDelay" -NoNewWindow -RedirectStandardOutput "${LogDir}\watch-out.log" -RedirectStandardError "${LogDir}\watch-err.log" -PassThru
...
$CeleryProcess = Start-Process -FilePath "celery" -ArgumentList "-A render.celery worker -Q rendering --pool=solo --loglevel debug --hostname=$Hostname --without-gossip --without-mingle --heartbeat-interval=$HeartbeatInterval --logfile=$LogDir/celery.log" -NoNewWindow -PassThru
Set-ProcessPriority -ProcessId $CeleryProcess.id -Priority High
