...
def read_model_parameters(ClockStruct,Soil,Crop,planting_dates,harvest_dates):
    ...
    *Returns:*
    ...
    while Soil.zSoil < Crop.Zmax+0.1:
        for i in reversed(range(len(Soil.profile))):
            if Soil.profile.dz.iloc[i] < 0.25:
                Soil.profile.dz.iloc[i] += 0.1
                Soil.fill_nan()
                break
...
def read_irrigation_management(ParamStruct,IrrMngt,ClockStruct):
    ...
    for i in range(ParamStruct.NCrops):
        ...
        if Crop.IrrMngt.IrrMethod == 3:
            ...
            Crop.IrrSchd = df.values

        else:

            Crop.IrrMngt.Schedule = np.zeros(len(ClockStruct.TimeSpan))


        Crop.IrrMngt.SMT = np.array(Crop.IrrMngt.SMT,dtype=float)

        irr_mngt_struct = IrrMngtStruct(len(ClockStruct.TimeSpan))
        for a,v in Crop.IrrMngt.__dict__.items():
            if hasattr(irr_mngt_struct,a):
                irr_mngt_struct.__setattr__(a,v)


        #Crop.IrrMngtStruct = irr_mngt_struct

        ParamStruct.CropList[i] = Crop


    return ParamStruct, irr_mngt_struct
...
def read_field_management(ParamStruct,FieldMngt,FallowFieldMngt):
    ...
    *Returns:*
    ...
    for i in range(ParamStruct.NCrops):
        ...
        # get crop object
        ParamStruct.CropList[i].FieldMngt = FieldMngt
...
def compute_variables(ParamStruct,weather_df,ClockStruct,acfp=pathlib.Path(os.path.abspath(aquacrop.__file__)).parent):
    ...
    ParamStruct.python_crop_list = crop_list
    ParamStruct.python_fallow_crop = Fallow_Crop



    ParamStruct.Seasonal_Crop_List = []
    ParamStruct.crop_name_list = []
    for crop in crop_list:
        ParamStruct.crop_name_list.append(crop.Name)
    ...
    ParamStruct.Fallow_Crop_Name = Fallow_Crop.Name
...
def read_model_initial_conditions(ParamStruct,ClockStruct,InitWC):
    ...
    *Returns:*
    ...
    if ClockStruct.SeasonCounter == -1:
        ...
        crop = ParamStruct.python_crop_list[ClockStruct.SeasonCounter]
