...
# Cell
def check_groundwater_table(ClockStruct,Soil,InitCond,ParamStruct):
    """
    ...
    *Returns:*
    ...
    if (ParamStruct.WaterTable == 1) and (ParamStruct.WTMethod == 'Variable'):

        # Update groundwater conditions for current day
        NewCond.zGW = float(ParamStruct.zGW[ClockStruct.TimeStepCounter])
...
def rainfall_partition(P,InitCond,
    ...
    *Arguments:*
    ...
    `FieldMngt_SRinhb`: `str` : 'Y','N'

    `FieldMngt_Bunds`: `str` : Bunds present ('Y','N')

    `FieldMngt_zBund`: `float` : Bund height

    `FieldMngt_CNadjPct`: `float` : curve number adjustment
    ...
    *Returns:*
    ...
    ## Calculate runoff ##
    if (FieldMngt_SRinhb == 'N') and ((FieldMngt_Bunds == 'N') or (FieldMngt_zBund < 0.001)):
...
def infiltration(prof,InitCond,Infl,Irr,IrrMngt_AppEff,
                 FieldMngt_Bunds,FieldMngt_zBund,
    ...
    if FieldMngt_Bunds == 'Y':
        ...
        if FieldMngt_zBund > 0.001:
            ...
            if InflTot > 0:
                ...
                if NewCond.SurfaceStorage > (FieldMngt_zBund*1000):
                    # Water overtops bunds and runs off
                    RunoffIni = NewCond.SurfaceStorage - (FieldMngt_zBund*1000)
                    # Surface storage equal to bund height
                    NewCond.SurfaceStorage = FieldMngt_zBund*1000
                else:
        ...
        if FieldMngt_Bunds == 'Y':
            if FieldMngt_zBund > 0.001:
                ...
                if NewCond.SurfaceStorage > (FieldMngt_zBund*1000):
                    ...
                    Runoff = RunoffIni+(NewCond.SurfaceStorage-(FieldMngt_zBund*1000))
...
# Cell
def capillary_rise(Soil,InitCond,FluxOut,ParamStruct_WaterTable):
    """
    ...
    *Returns:*
    ...
    if ParamStruct_WaterTable == 0: # No water table present
        ...
        zTopLayer = 0
        for layeri in np.sort(np.unique(Soil.Profile.Layer)):
            # Calculate layer thickness
            l_idx = np.argwhere(Soil.Profile.Layer==layeri).flatten()
...
FieldMngt_Mulches,FieldMngt_fMulch,FieldMngt_MulchPct,
InitCond,Et0,Infl,Rain,Irr,GrowingSeason):
    ...
    *Returns:*
    ...
    if NewCond.SurfaceStorage < 0.000001:
        if FieldMngt_Mulches == 'N':
            # No mulches present
            EsPotMul = EsPot
        elif FieldMngt_Mulches == 'Y':
            # Mulches present
