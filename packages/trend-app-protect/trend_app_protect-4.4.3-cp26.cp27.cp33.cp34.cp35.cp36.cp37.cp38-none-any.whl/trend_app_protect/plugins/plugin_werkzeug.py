...
def hook_werkzeug_input_params(run_hook, get_agent_func, timer):
    ...
    if not (
        isinstance(BaseRequest.args, cached_property)
        and isinstance(BaseRequest.form, cached_property)
    ):
        log.warning("Werkzeug BaseRequest values not cached_properties")
    ...
    class WrappedProperty(object):
        ...
        def __get__(self, obj, objtype=None):
            ...
            if first_call:
                # Standard ImmutableMultiDict can be converted to a list-y
                # dict:
                try:
    ...
    BaseRequest.args = WrappedProperty(BaseRequest.args)
    BaseRequest.form = WrappedProperty(BaseRequest.form)
