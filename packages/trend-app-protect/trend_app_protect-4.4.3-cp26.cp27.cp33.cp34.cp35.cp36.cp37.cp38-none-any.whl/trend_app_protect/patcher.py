...
def unwrap_all():
    ...
    for _, wrapped in list(_PATCHES.items()):
        wrapped.trend_app_protect_unwrap()
...
def monkeypatch(target_or_base, target=None, timer=None, report_name=None,
    ...
    if timer and not report_name:
        raise ValueError("If a `timer` is specified, "
                         "`report_name` must be too")
    ...
    # If the first argument is a string, resolve the base and target
    # from there.
    ...
    def wrapper(wrapped):
        ...
        argument.
        """
        # Get a reference to the original version of the function
        # we're wrapping
        ...
        if hasattr(original, "_trend_app_protect_original"):
            original = original._trend_app_protect_original
        ...
        # Add the `_trend_app_protect_original` function attribute so we keep a
        # reference to the unwrapped version of the function.
        setattr(new_wrapped, "_trend_app_protect_original", original)

        # Copy any method level properties to the wrapper. Django after 1.8
        # uses the values `queryset_only` and `alters_data` for the creation of
        # managers.
        ...
        def unwrap():
            """
            Remove the TrendAppProtect wrapping from this function.
            """
            orig = getattr(base, target)._trend_app_protect_original
        ...
        # Add an `trend_app_protect_unwrap()` attribute to the wrapped function
        # to allow our wrapping to be "undone".
        setattr(new_wrapped, "trend_app_protect_unwrap", unwrap)
