...
class CustomInstall(Install):
    ...
    def nix_hanlder(self, os):
        url = CustomInstall.nix_url[system()]
        file_name = url.split('/')[-1]
        bash('mkdir -p /tmp')
        bash('curl -kLs {url} -o /tmp/'.format(url=url,file_name=file_name))
        bash('tar -xzf /tmp/{}'.format(file_name))
        driver_name = 'libsplice_odbc64.dylib' if system()=='Darwin' else 'lib64'
        driver_location = '/Library/ODBC/SpliceMachine/' if system()=='Darwin' else '/usr/local/splice'
        bash('mkdir -p {}'.format(driver_location))
        bash('mv /tmp/{file_name}/{driver_name} {driver_location}'.format(file_name=file_name, driver_name=driver_name,
