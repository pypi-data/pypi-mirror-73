...
class AnalysisWorkstep(Workstep):
    ...
    def _get_display_range(self):
        ...
        if not display_range:
            return None
        start = datetime.datetime.fromtimestamp(display_range['start'] / 1000)
        end = datetime.datetime.fromtimestamp(display_range['end'] / 1000)
    ...
    def _set_display_range(self, display_start_end, check_investigate=True):
        ...
        auto_update['offset'] = end_ts - datetime.datetime.now().timestamp() * 1000
    ...
    def _get_investigate_range(self):
        ...
        if investigate_range is None:
            return None
        start = datetime.datetime.fromtimestamp(investigate_range['start'] / 1000).isoformat()
        end = datetime.datetime.fromtimestamp(investigate_range['end'] / 1000).isoformat()
