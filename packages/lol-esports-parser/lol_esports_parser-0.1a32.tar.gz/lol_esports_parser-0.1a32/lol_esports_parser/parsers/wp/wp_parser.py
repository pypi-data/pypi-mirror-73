...
def parse_wp_game(
    raw_data: dict, patch: str = None, add_names=True, discrete_mode=True, raise_missing_events=False
) -> LolWpGame:
    ...
    for players_list in data["plList"]:
        for idx, key_player_tuple in enumerate(players_list.items()):
            ...
            if player_dict.get("stats"):
                ...
                # Adding stats perks
                runes.extend(
                    [LolGamePlayerRune(id=player_dict["stats"].get(f"perk{i}"), slot=i + 6,) for i in range(0, 3)]
                )
                ...
                end_of_game_stats = LolGamePlayerEndOfGameStats(
                    ...
                    cs=(
                        player_dict["stats"].get("minionsKilled") or player_dict["stats"].get("totalMinionsKilled") or 0
                    )
                    + player_dict["stats"].get("neutralMinionsKilled"),  # In older games, it’s totalMinionsKilled
            ...
            try:
                ...
                logging.warning(f"Incoherent participantId fields for wp game {game_id}")
    ...
    for bans_list in data["bpList"]["bans"]:
        for ban in bans_list:
            if type(ban) != dict:
                logging.info(f"Cannot parse bans for game {game_id}.")
                continue
...
def add_timeline(game: LolWpGame, data: dict, add_names) -> LolWpGame:
    ...
    try:
        ...
        logging.info(f"No frames data for game {game['sources']['wp']['id']}")
        return game
        ...
        for event in frame["events"]:
            ...
            # Item buying, selling, and undoing
                ...
                if not event.get("participantId"):
                    logging.debug(f"Item destroyed with participantId=0 or None")
                    continue
    ...
    if frames_dict.get("positionInfo"):
        for player_key in frames_dict["positionInfo"]:
            ...
            try:
                ...
                logging.warning(f"Cannot get position info for player {player['inGameName']}/{player['role']}")
                continue
