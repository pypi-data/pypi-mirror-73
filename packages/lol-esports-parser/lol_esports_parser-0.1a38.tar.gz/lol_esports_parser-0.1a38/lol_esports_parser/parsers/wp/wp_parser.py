        ...
        for event in frame["events"]:
            ...
            if event["type"] == "ELITE_MONSTER_KILL":
                if event["killerId"] < 1:
                    ...
                    lol_esports_parser_logger.info(f"Epic monster kill with killer id 0 found, likely Rift Herald killing itself.")
                    continue
    ...
    if frames_dict.get("positionInfo"):
        for player_key in frames_dict["positionInfo"]:
            ...
            try:
                ...
                lol_esports_parser_logger.warning(f"Cannot get position info for player {player['inGameName']}/{player['role']}")
                continue
