...
def get_service_by_title(title, check_author=False):
    ...
    service = (
        get_db()
            .execute(
            # "SELECT s.id, s.title, body, created, author_id, username"
...
def service(service_name):
    ...
    return render_template("service/services.html", methods=methods, service_name=service_name)
...
def get_method(service_name, method_name, check_author=False):
    ...
    method = (
        get_db()
            .execute(
            "SELECT m.id, m.title, m.body, m.status_code, m.delay, m.supported_method, m.headers, m.created, m.author_id, m.service_id, username"
...
def create_method(service_name):
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            db.execute(
                "INSERT INTO method (title, body, status_code, delay, supported_method, headers, author_id, service_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def method(method_name, service_name):
    ...
    method_response = db.execute(
        "SELECT body, status_code, delay, supported_method, headers FROM method WHERE service_id = ? AND title = ?", (service["id"], method_name,),
    ).fetchone()
    ...
    if supported_method != request.method:
        return render_template("service/method_not_allowed.html", method=method, service_name=service_name, method_name=method_name, current_method=request.method)
...
def update_method(service_name, method_name):
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                "UPDATE method SET title = ?, body = ?, status_code = ?, delay = ?, supported_method = ?, headers = ? WHERE id = ?", (title, body, status_code, delay, supported_method, headers, method['id'])
            )
            db.commit()
...
def delete_method(service_name, method_name):
    ...
    db = get_db()
    db.execute("DELETE FROM method WHERE title = ? AND service_id = ?", (method_name, method_for_delete['service_id']))
    db.commit()
