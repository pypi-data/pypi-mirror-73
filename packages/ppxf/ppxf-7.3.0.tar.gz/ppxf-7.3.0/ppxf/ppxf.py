...
from .capfit import capfit, lsqlin, lsqbox, cov_err

################################################################################

def lsqlin_cvxopt(A_lsq, b_lsq, A_ineq, b_ineq, A_eq, b_eq, initvals=None):
...
class ppxf:
    ...
    .. code-block:: python
        ...
        pp = ppxf(self, templates, galaxy, noise, velscale, start,
                 ...
                 linear_method='lsqbox', mask=None, method='capfit', mdegree=0,
    ...
    linear:
        ...
        are zero.
    linear_method: {'nnls', 'lsqbox', 'lsqlin', 'cvxopt'} optional
        Method used for the solution of the linear least-squares subproblem
        to fit for the templates weights (default 'lsqbox' fast box-constrained).

        The computational speed of the four alternative linear methods depends
        on the size of the problem, with 'lsqbox' generally being the fastest
        without linear inequality constraints and 'cvxopt' the fastest with
        linear inequality constraints. The 'nnls' option (the default unti now)
    ...
    def __init__(self, templates, galaxy, noise, velscale, start,
        ...
        assert linear_method in ['nnls', 'lsqbox', 'lsqlin', 'cvxopt'], \
            "`linear_method` must be 'nnls', 'lsqbox', 'lsqlin' or 'cvxopt'"
    ...
    def set_linear_constraints(self, constr_templ, constr_kinem, method):
        ...
        if constr_templ != {}:
            if "A_ineq" in constr_templ:
                assert self.linear_method not in ['lsqbox', 'nnls'], \
                    "`A_ineq` not supported with `linear_method` = 'lsqbox' or 'nnls'"
...
################################################################################
    ...
    def solve_linear(self, A, b, npoly):
        ...
        if n == 1:  # A is a column vector, not an array
            ...
            if self.linear_method == 'lsqlin':
                soluz = lsqlin(A, b, self.A_ineq_templ, self.b_ineq_templ,
                               self.A_eq_templ, self.b_eq_templ)
            elif self.linear_method == 'cvxopt':
                res = lsqlin_cvxopt(A, b, self.A_ineq_templ, self.b_ineq_templ,
                ...
                if self.linear_method == 'lsqbox':
                    lb = np.zeros(n)
                    lb[:npoly] = -np.inf
                    soluz = lsqbox(A, b, [lb, np.inf], self.x0)
                    self.x0 = soluz
