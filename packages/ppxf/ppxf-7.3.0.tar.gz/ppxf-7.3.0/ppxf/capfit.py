...
def lsqbox(A, b, bounds, x=None, cond=None, verbose=False, ftol=1e-10):
    """
    ...
    ftol:
        Safety criterion to stop iterating. The actual accuracy of the
        minimum calculation is generally better than ftol and the final
        accuracy is nearly insensitive to this value.
    ...
    while np.any(~f) and (chi2old - chi2new > ftol*chi2old):    # Loop A
        ...
        wd = A.T @ (b - A @ x)*d    # Negative of gradient
        j = np.argmax(wd)
        if wd[j] > 0:
            f[j] = bad = alpha = 1
            while np.any(bad) and (alpha > 0):    # Loop B
                z = x.clip(*bounds)
                z[f] = linalg.lstsq(A[:, f], b - A[:, ~f] @ x[~f], cond=cond)[0]
                bad = (z < lb) | (ub < z)
                if np.any(bad) and np.any(x != z):
                    bnd = np.select([z < lb, ub < z], bounds)
                    rat = (bnd[bad] - x[bad])/(z[bad] - x[bad])
                    k = np.argmin(rat)
                    alpha = rat[k]
                    x[f] += alpha*(z[f] - x[f])
                    x[bad][k] = bnd[bad][k]    # Place blocking constraint exactly on bound
                    f = (lb < x) & (x < ub)
                    if verbose:
                        print(f"b {ita:6d}**   alpha: {alpha:.4f}")
            x, chi2old, chi2new = z, chi2new, chi2(b - A @ z)
        else:
            if verbose:
        ...
        if verbose:
            ita += 1
            print(f"b {ita:6d} {chi2new/2:15.4e} {(chi2old - chi2new)/2:15.4e}")
...
def lsqlin(A_lsq, b_lsq, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
    ...
    """
    Linear least-squares with linear equalities and/or inequalities constraints.
    ...
    <https://doi.org/10.1137/1.9781611971217>`_
    ...
    # Eliminate equality constraints with change of variables and pivoting
    if A_eq is not None:
...
class capfit:
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
        ...
        while(True):
            ...
            if bounds_only:
                h = lsqbox(A, b, bounds=bounds-p1, cond=cond)
            else:
