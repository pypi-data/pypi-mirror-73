...
from spintop.services.bigquery.target_bigquery import emit_state, persist_lines_job
...
class BigQuerySingerTarget(AbstractSingerTarget):
    ...
    def send_messages(self, messages_str, request_id=None, stream_name=None):
        state = persist_lines_job(self.project_id, self.dataset_id, messages_str, truncate=self.truncate, validate_records=self.validate_records)
        emit_state(state)
        return BigQueryAnalyticsResponse(
            count = len(messages_str),
