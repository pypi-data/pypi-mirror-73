...
class QaBase:
    ...
    def conn_mongo(self, **kwargs):
        ...
        if debug():
            qa_name = self.__class__.__name__
            stack = inspect.stack()
            self.qa_report_file = os.path.join(os.path.dirname(get_stack_frame(stack)[1]), 'QaReports', '{}.html'.format(qa_name))
        else:
            qa_name = self.__module__ + self.__class__.__name__
            self.qa_report_file = os.path.join(os.path.expanduser('~'), 'QaReports', '{}.html'.format(qa_name))

        self.collection_qa = self.qa_db[qa_name]
