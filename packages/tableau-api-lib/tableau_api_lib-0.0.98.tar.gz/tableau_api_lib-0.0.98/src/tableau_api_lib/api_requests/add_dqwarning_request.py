...
class AddDQWarningRequest(BaseRequest):
    def __init__(self,
                 ts_connection,
                 warning_type,
                 message,
                 status=None):
        """
        Builds the request body for Tableau Server REST API requests adding data quality warnings.
    ...
    @property
    def required_param_values(self):
        return [
            self._warning_type,
            self._message
        ]
    ...
    def base_update_column_request(self):
        self._request_body['dataQualityWarning'].update(self._get_parameters_dict(self.required_param_keys,
                                                                                  self.required_param_values))
