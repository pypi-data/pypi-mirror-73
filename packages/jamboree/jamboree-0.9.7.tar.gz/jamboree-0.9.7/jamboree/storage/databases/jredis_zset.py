...
class RedisDatabaseZSetsConnection(DatabaseConnection):
    ...
    def save_many(
        self, query, data: Dict[str, float] = {}, abs_rel="absolute"
    ):
        """ 
            # Save Many
    ...
    def _delete_all(self, _hash: str):
        ...
        with self.connection.lock(rlock):
            keys = self.connection.zrange(
                relative_time_key, 0, -1, withscores=True
            )
        ...
        with self.connection.pipeline() as pipe:
            ...
            while True:
                try:
                    ...
                    akeys = pipe.zrange(
                        absolute_time_key, 0, -1, withscores=True
                    )
                    pipe.execute()
                    break
                except redis.exceptions.WatchError:
        ...
        with self.connection.pipeline() as pipe:
            while True:
                try:
                    ...
                    keys = pipe.zrange(
                        _current_key, -limit, -1, withscores=True
                    )
                    if len(keys) == 0:
                        return {}
                    pipe.execute()
    ...
    def _reset_count(self, query: dict, mongo_data: List[Dict]):
        ...
        serialized_mongo = [
            orjson.dumps(mon, option=orjson.OPT_SERIALIZE_NUMPY)
            for mon in mongo_data
        ]
        # rlock = f"{_hash}:lock"
        with self.connection.pipeline() as pipe:
            while True:
