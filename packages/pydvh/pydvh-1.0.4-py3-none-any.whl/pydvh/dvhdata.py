...
class DVHData:
    ...
    def to_cumulative(self, eclipse_complied=False):
    ...
    def _get_position_in_volume_array(self, volume, volume_units):
        ...
        if volume_units == self.volume_unit:
            pos = len(self.volume_array) - np.searchsorted([*reversed(self.volume_array)], volume)
            return pos

        if volume_units == '%':
            if (self.volume_unit == 'cm3') or (self.volume_unit == 'mm3'):
                volume_in_absolute = volume * 0.01 * self.total_volume
                pos = len(self.volume_array) - np.searchsorted([*reversed(self.volume_array)], volume_in_absolute)
                return pos

        if (volume_units == 'cm3') or (volume_units == 'mm3'):
            if self.volume_unit == '%':
                volume_in_relative = volume * 0.01 / self.total_volume
                pos = len(self.volume_array) - np.searchsorted([*reversed(self.volume_array)], volume_in_relative)
                return pos

        if volume_units == 'cm3':
            if self.volume_unit == 'mm3':
                volume_in_mm3 = volume * 0.1 * 0.1 * 0.1
                pos = len(self.volume_array) - np.searchsorted([*reversed(self.volume_array)], volume_in_mm3)
                return pos

        if volume_units == 'mm3':
            if self.volume_unit == 'cm3':
                volume_in_cm3 = volume / (0.1 * 0.1 * 0.1)
                pos = len(self.volume_array) - np.searchsorted([*reversed(self.volume_array)], volume_in_cm3)
                return pos
        return -1
