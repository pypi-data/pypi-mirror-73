...
class DVHFile:
    ...
    def get_dvh_by_name(self, structure):
        dvhs = [*(x for x in self._dvhs if x.structure == structure)]
        if not dvhs:
            return None
        else:
            return dvhs[0]
    ...
    def from_file_eclipse(cls, filename):
        ...
        dvhs = cls._get_dvhs(structure_names, lines, dvhfile_type)
    ...
    @classmethod
    def _get_dvh(cls, structure: str, partition, lines, dvh_type: DVHType) -> object:
        dose_array = []
        volume_array = []
        ...
        return DVHData.from_dvh_file(structure=structure,
                                     dvh_type=dvh_type,
