...
RFXtrx.
TP : Creation of Cartelec Packet Type
"""
...
def parse(data):
    ...
    if data[1] == 0x01:
        ...
        if data[2] == 0x02:
            # Usage Encoder 2 counters
            pkt = CartelecEncoder()
        elif data[2] == 0x03:
            # Usage Linky
            pkt = CartelecLinky()
        else:
            return None  # Case Teleinformation historique not treated
    elif data[1] == 0x71:
        pkt = RfxMeter()
    else:
        return None
...
###############################################################################
# Cartelec class ( One per subtype)
###############################################################################


class CartelecEncoder(SensorPacket):
    """
    ...
    def __str__(self):
        return ("CartelecEncoder [subtype={0}, seqnbr={1}, id={2}, counter1={3}, " +
                "counter2={4}, battery={8}, rssi={9}]") \
            .format(self.type_string, self.seqnbr, self.id_string,
                    self.counter1, self.counter2, self.battery, self.rssi)

    def __init__(self):
        """Constructor"""
        super(CartelecEncoder, self).__init__()
        self.id1 = None
        self.id2 = None
        self.id3 = None
        self.id4 = None
...
class CartelecLinky(SensorPacket):
    """
    Data class for the Cartelectronic packet type
    """
    ...
    def __str__(self):
        return ("CartelecLinky [subtype={0}, seqnbr={1}, id={2}, conswatthours={3}, " +
                "prodwatthours={4},tarif_num={5},voltage={6},currentwatt={7},teleinfo_ok={8}, " +
                " battery={9}, rssi={10}]") \
            .format(self.type_string, self.seqnbr, self.id_string, self.conswatthours,
                    self.prodwatthours, self.tarif_num, self.voltage, self.currentwatt,
                    self.teleinfo_ok, self.battery, self.rssi)

    def __init__(self):
        """Constructor"""
        super(CartelecLinky, self).__init__()
        self.id1 = None
        self.id2 = None
        self.id3 = None
        self.id4 = None
