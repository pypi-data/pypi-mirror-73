    ...
    while ((k<= (len(node_list)+profile_iter)) & (ultima !=2) ):
      ...
      if(len(aux) != 0):  
        ...
        if path.exists(os.path.join(filepath,filename)):
          ...
          df2.to_csv(path_of_buf = os.path.join(filepath,filename),index=False)         
        if len(list_users_noInfo) !=0:
          nodes = pd.DataFrame()
          nodes["nodos"] = list_users_noInfo
          if path.exists(os.path.join(filepath,"node_without_",filename)):
            nodes.to_csv(os.path.join(filepath,"node_without_",filename), mode ="a",index=False, header=False)          
          else:
             nodes.to_csv(os.path.join(filepath,"node_without_",filename), index=False)
        k = k+profile_iter        
        if ultima ==1:          
          ultima =2
      else:
...
def abnormal_categories_profiles2(data_join):
