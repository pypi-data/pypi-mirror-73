...
def compute_theta_vr(path, time=1):
    row1, row2 = path[time], path[time-1]
    diff1 = np.array([row2[0] - row1[0], row2[1] - row1[1]])
    theta1 = np.arctan2(diff1[1], diff1[0])
    vr1 = np.linalg.norm(diff1) / (1 * 0.4)
    # if vr1 < 0.05:
    #     return 0, 0
    return theta1, vr1

def dataset_plots(input_file, n_theta=64, vr_max=1.2, vr_n=10, obs_length=9):
