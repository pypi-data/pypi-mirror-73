...
def Execute_AppX64(Paths,VE_Path,VE=False):
    Paths="\""+Paths+"\""
    #Paths = "\"" + Paths + "\""
    Text="import java.io.BufferedReader;\nimport java.io.IOException;\nimport java.io.InputStreamReader;\n\npublic class RunApp {\n"+"public static void main(String args[]){\n"+"try {\nRuntime p = Runtime.getRuntime();\np.exec(""\"Elevatex64.exe \"+"""+Paths+");\n} catch (IOException ex) \n{ex.printStackTrace();}\n"+"}}"
...
def Execute_AppX32(Paths,VE_Path,VE=False):
    Paths = "\"" + Paths + "\""
    # Paths = "\"" + Paths + "\""
    Text = "import java.io.BufferedReader;\nimport java.io.IOException;\nimport java.io.InputStreamReader;\n\npublic class RunApp {\n" + "public static void main(String args[]){\n" + "try {\nRuntime p = Runtime.getRuntime();\np.exec(""\"Elevatex32.exe \"+""" + Paths + ");\n} catch (IOException ex) \n{ex.printStackTrace();}\n" + "}}"
    Java_Save.Save_Java(Text, "RunApp")
    if (VE == True):
        VE_Java_Execute.CompileJava(VE_Path + "javac.exe", "RunApp.java", "")
        VE_Java_Execute.ExecuteJava(VE_Path + "java.exe", "RunApp", "")
...
#Execute_AppX32("D:\\\Projects\\\Python\\\Phoenix--Lang\\\Phoenix\\\\temp.exe")