    ...
    def minify_meth_files(self, processed_dir=None, cpg_min_dir=None, beta_cutoff=0.5):
        ...
        for f in cpg_files:
            ...
            try:
                ...
                df_tmp['gene_name'] = gene_symbols
                df_tmp = df_tmp.fillna(0)
                grouped_meth_df = df_tmp.groupby('gene_name')
                gene_betas = []
                gene_names = []
                for g in grouped_meth_df:
                    gene = g[0]
                    beta_values = g[1]['Beta_value'].values
                    beta_val = len(np.where(beta_values > beta_cutoff)[0]) / len(beta_values)
                    gene_betas.append(beta_val)
                    gene_names.append(gene)

                    i += 1

                meth_df = pd.DataFrame()
                meth_df['id'] = gene_names
                meth_df[f] = gene_betas
