...
class ProjectConfiguration(ConfigurationBase):
    ...
    def save(self, config, name, action='create'):
        ...
        idds_dir = os.path.join(proj_path, 'idds')
        os.makedirs(proj_path, exist_ok=True)        
        os.makedirs(scripts_dir, exist_ok=True)
        os.makedirs(config_dir, exist_ok=True)
        os.makedirs(idds_dir, exist_ok=True)
        ...
        idds_config_path = os.path.join(idds_dir, kiDDSConfigName)
        idds_search_space_path = os.path.join(idds_dir, kiDDSSearchSpaceName)
        ...
        # save configurations for idds workflow
        idds_config = self.to_idds_config(project_config)
        with open(idds_config_path, 'w') as idds_config_file:
            json.dump(idds_config, idds_config_file, indent=2)
            print('INFO: Created iDDS configuration: {}'.format(idds_config_path))
    ...
    def to_idds_config(config):
        ...
        idds_config['steeringExec'] = ('run --rm -v \"$(pwd)\":{workdir} {container} '
        '/bin/bash -c \"{cmd}\"').format(workdir=kWorkDir, container=kDefaultContainer, cmd=generator_cmd)
        idds_config['evaluationExec'] = 'hpogrid idds_run {project_name}'.format(project_name=project_name)
        ...
        idds_config['nParallelEvaluation'] = config['hpo_config']['max_concurrent']
