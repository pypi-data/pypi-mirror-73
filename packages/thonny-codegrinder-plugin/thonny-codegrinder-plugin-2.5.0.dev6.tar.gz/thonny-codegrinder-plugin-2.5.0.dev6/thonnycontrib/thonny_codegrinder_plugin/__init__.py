...
failedAt = None
failedDelta = datetime.timedelta(seconds=10)
...
def mustLoadConfig():
    ...
    if home == '':
        ...
        tkinter.messagebox.showerror('Fatal error',
            'Unable to locate home directory, giving up')
        return False
...
def checkVersion():
    ...
    if server is None:
        failNow()
        return False
    elif versionTuple(server.grindVersionRequired) > versionTuple(__version__):
        failNow()
        ...
        return False
    elif versionTuple(server.grindVersionRecommended) > versionTuple(__version__):
...
def _codegrinder_login_handler():
    ...
    session = getNamedTuple('/users/session', {'key':fields[3]})
    if session is None:
        tkinter.messagebox.showerror('Login failed',
            str(err) + '\n\n' +
...
def _codegrinder_run_tests_handler():
    ...
    if os.path.exists(os.path.join(problemDir, 'tests')):
        ...
        tkinter.messagebox.showwarning('Unknown problem type',
...
def _codegrinder_save_and_sync_handler():
    ...
    raw = gatherStudent(now, problemDir)
    if raw is None:
        return
    (problemType, problem, assignment, commit, dotfile) = raw
    commit['action'] = ''
...
def show_instructions(problemDir):
    ...
    text = thonny.get_workbench().get_view('AssistantView').text
    text.clear()
    text.direct_insert("end", doc)
    thonny.get_workbench().show_view('AssistantView', set_focus=False)
