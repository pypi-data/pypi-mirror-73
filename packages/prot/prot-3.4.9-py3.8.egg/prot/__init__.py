...
if _os.path.split(_os.path.split(__file__)[0])[1] == 'lightprot':
	status.append('light')
	__version__ = __version__.replace('unofficial', 'light')
	try:
...
class Progress(object):
	...
	def __init__(self, val=0, minVal=0, maxVal=100, msg='completed', msgForm=None, replaces=[], color=None, inline=False):
		...
		self.max = maxVal
		self.msg = msg
		self.inline = inline if _sys.stdout == _sys.__stdout__ else False
		self.color = color
		if not msgForm:
...
class VersionString(str):
	...
	def upgrade(self, target='micro', maxMinor=9, maxMicro=9):
		...
		if self.minor is not None and self.minor >= maxMinor:
			...
			major = False
		if self.micro is not None and target == 'micro' and not (minor or major):
			self.micro += 1
		if self.minor is not None and (target == 'minor' or minor) and not major:
			if self.micro is not None:
				self.micro = 0
			self.minor += 1
		if self.major is not None and target == 'major' or major:
			if self.micro is not None:
				self.micro = 0
