...
class QPDWorkflowContext(WorkflowContext):
    def __init__(self, operators: Operators, dfs: Dict[str, Any]):
        self._operators = operators
        self._dfs = DataFrames({k: operators.to_df(v) for k, v in dfs.items()})
...
class QPDTask(TaskSpec):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._kwargs = kwargs
        assert_or_throw(
            output_n <= 1, NotImplementedError("multiple output tasks"),
        )
        inputs = [
    ...
    def execute(self, ctx: TaskContext) -> None:
        ...
        op = ctx.workflow_context.operators
...
class Create(QPDTask):
    ...
    def create(self, op: Operators, ctx: TaskContext) -> Any:  # pragma: no cover
...
class CreateColumn(Create):
    ...
    def create(self, op: Operators, ctx: TaskContext) -> Any:
