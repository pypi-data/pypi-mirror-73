...
class RelayTransformer(nn.Module):
    def __init__(self, dim, max_seq_len, depth, causal = False, heads = 8, window_size = 256, relay_tokens_per_window = 2, depth_start_relay_attn = None):
        super().__init__()
        ...
        self.relay_attns = nn.ModuleList([None if (ind + 1) < depth_start_relay_attn else Residual(PreNorm(dim, SelfAttention(dim, heads = heads, causal = causal, window_size = window_size, full_attention = True))) for ind in range(depth)])
...
class RelayTransformerLM(nn.Module):
    def __init__(self, num_tokens, dim, max_seq_len, depth, causal = False, heads = 8, window_size = 256, relay_tokens_per_window = 2, depth_start_relay_attn = None):
        super().__init__()
        ...
        self.transformer = RelayTransformer(dim, max_seq_len, depth, causal = causal, heads = heads, window_size = window_size, relay_tokens_per_window = relay_tokens_per_window, depth_start_relay_attn = depth_start_relay_attn)
