...
class FieldsConverter:
    ...
    def get_columns(self, table_name, db="mysql", type=None, table_from=True):
        if table_from:
            fields = self.get_table_scheme_mysql(table_name, self.meta_from)
        else:
            fields = self.get_table_scheme_mysql(table_name, self.meta_to)
        columns = [f["column_name"] for f in fields]
        types = [f["data_type"] for f in fields]
