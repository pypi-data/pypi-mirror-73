...
c = pc.Counter('nbapis_request_count', 'Count the total number of incoming request', ['label', 'status_code', 'cid'])
g = pc.Gauge('nbapis_request_duration', 'Gauge the duration of incoming request', ['label', 'status_code', 'cid'])
...
class NBApi(FastAPI):
    def __init__(self, title: str, cors_origins: list = []):
        ...
        async def http_middleware(request: Request, call_next):
            ...
            c.labels(label=request.state.ctx.label, status_code=resp.status_code, cid=request.state.ctx.cid).inc()
            g.labels(label=request.state.ctx.label, status_code=resp.status_code, cid=request.state.ctx.cid).set(time.time() - ts)
            return resp
