...
def VR_test(data,periods=2,return_p=False,with_drift=True,assume_hete=True):
    ...
    The test detects deviation from normal distribution in Continuously compounded asset returns by comparing estimated variances from different holding periods
    ...
    for ss in periods: #loop for all the selected return periods
        ...
        if with_drift: #for small period returns such as daily returns the drift should be small and thus the difference is insignificant for larget size n 
