...
class Augment:
    def __init__(self,difficulty = 0.0, generate_heatmaps = False, heatmaps_scale = 1.0):
    ...
    def value_from_type(self, augmentation_type):
    ...
    def noisy_value_from_type(self, augmentation_type):
        if len(self._random_multipliers) == 0:
            self._random_multipliers.extend(
                    list(np.random.normal(size=(len(self._augmentation_ranges),))))
        return self._random_multipliers.pop() * self.value_from_type(augmentation_type)
