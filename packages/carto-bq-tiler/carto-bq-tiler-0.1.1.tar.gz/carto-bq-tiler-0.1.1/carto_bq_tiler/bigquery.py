...
from .settings import (BIGQUERYRC_FILE_PATH, CARTO_PARTITION_DEFAULT_MAX, CARTO_PARTITION_DEFAULT_MIN,
                       ...
                       TILESET_TABLE_SCHEMA, TILESET_VALID_VERSIONS)
...
class BigQueryClient:
    ...
    def exists_tileset(self, tileset_name):
        ...
        try:
            ...
            return True if tileset_version in TILESET_VALID_VERSIONS else False
    ...
    def upload_tileset_metadata_and_label(self, tileset_name, metadata):
        ...
        table = self._client.get_table(table_id)
        table.description = json.dumps(metadata)
        table.labels = {TILESET_LABEL: TILESET_VALID_VERSIONS[-1]}
        table = self._client.update_table(table, ['description', 'labels'])

    def get_tileset_metadata(self, tileset_name):
        table_id = '{project}.{tileset_name}'.format(project=self.project, tileset_name=tileset_name)
        table = self._client.get_table(table_id)
        return json.loads(table.description)

    def list_tilesets(self):
        tables = []
        for dataset in self._client.list_datasets():
            for table in self._client.list_tables(dataset.dataset_id):
                if table.labels.get(TILESET_LABEL) and table.labels.get(TILESET_LABEL) in TILESET_VALID_VERSIONS:
                    tables.append({
    ...
    def exist_tilesets(self, tileset_names):
        tileset_names_ = [self.validate_tileset_name(tileset_name) for tileset_name in tileset_names]
        bigquery_tileset_names = [tileset['tileset_name'] for tileset in self.list_tilesets()]
    ...
    def get_tileset_name(self, tileset_name):
        tileset_name_ = self.validate_tileset_name(tileset_name)
        used_tileset_names = [tileset['tileset_name'] for tileset in self.list_tilesets()]
