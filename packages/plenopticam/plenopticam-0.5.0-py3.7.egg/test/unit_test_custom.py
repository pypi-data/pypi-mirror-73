...
class PlenoptiCamTesterCustom(DataDownloader, unittest.TestCase):
    ...
    def setUp(self):
        ...
        archive_fn = join(self.fp, basename(self.opex_url))
        self.download_data(self.opex_url) if not exists(archive_fn) else None
        self.extract_archive(join(self.fp, basename(self.opex_url)), self.opex_fnames_wht + self.opex_fnames_lfp)
    ...
    def test_custom_cal(self):
        ...
        for fn_lfp, fn_wht in zip(self.opex_fnames_lfp, self.opex_fnames_wht):
