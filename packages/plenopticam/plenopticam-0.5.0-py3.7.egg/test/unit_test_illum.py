...
class PlenoptiCamTesterIllum(DataDownloader, unittest.TestCase):
    ...
    def setUp(self):
        ...
        archive_fn = os.path.join(self.fp, os.path.basename(self.host_eu_url))
        self.download_data(self.host_eu_url) if not os.path.exists(archive_fn) else None
        fnames_illum = [file for file in ZipFile(archive_fn).namelist() if file.startswith('caldata') or file.endswith('lfr')]
        self.extract_archive(os.path.join(self.fp, os.path.basename(self.host_eu_url)), fnames_illum)
