...
class CentroidSorter(object):
    ...
    def _assign_mic_idx(self):
        ...
        for ly in range(self._lens_y_max):
            ...
            for lx in range(self._lens_x_max):    # actually -1 to account for first row center which is already found
                # get adjacent MIC
                ...
                if len(found_center) != 2:
                    if len(found_center) > 2:
                        ...
                        # when looking for last in row
                        if lx == self._lens_x_max-1 or self._pattern == 'rec':
                            break
