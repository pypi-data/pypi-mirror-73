...
class PitchEstimator(object):
    ...
    def _find_scale_max(self, precision=.1):
        ...
        if len(maxima) > 2:
            ...
            # exclude maxima on falling curve
            #start = np.where(np.diff(np.sign(np.gradient(y_new, inv_prec))) > 0)[0][0]+1
            start = int(1./precision)

            # analyze global and relative maxima in scale space
            val_max, arg_max = np.max(y_new), x_new[np.argmax(y_new)]
            rel_max, arg_rel_max = y_new[min(scipy.signal.argrelmax(y_new)[0])], \
                                   x_new[min(scipy.signal.argrelmax(y_new[start:])[0])+start]

            # use global maximum only if it's 5 times bigger than relative maximum
            scale_max = arg_max if val_max > 5*rel_max else arg_rel_max
