...
class LfpDevignetter(LfpMicroLenses):
    ...
    def main(self):
        ...
        if self.patch_mode and self.noise_lev > self.noise_th:
            # equalize color variances (Bayer pattern only)
            #self.compensate_bayer_colors() if 'awb_wht' in self.cfg.lfpimg else None
    ...
    def compensate_bayer_colors(self):

        ch1 = np.mean(self._wht_img[0::2, 1::2]) / np.mean(self._wht_img[0::2, 0::2])
        ch2 = np.mean(self._wht_img[1::2, 0::2]) / np.mean(self._wht_img[0::2, 0::2])
        ch3 = np.mean(self._wht_img[1::2, 1::2]) / np.mean(self._wht_img[0::2, 0::2])

        self._lfp_img[0::2, 1::2] *= ch1
        self._lfp_img[1::2, 0::2] *= ch2
        self._lfp_img[1::2, 1::2] *= ch3
