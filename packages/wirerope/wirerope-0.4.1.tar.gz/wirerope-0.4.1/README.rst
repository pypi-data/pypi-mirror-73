...
- g`wirerope.rope.WireRope` is a wrapper interface for python callable.
- Custom g`wirerope.wire.Wire` class provides user-defined behavior.
...
g`wirerope.rope.WireRope` is the wrapper for callables. By wrapping a
function with `WireRope` with a custom subclass of the g`wirerope.wire.Wire`
