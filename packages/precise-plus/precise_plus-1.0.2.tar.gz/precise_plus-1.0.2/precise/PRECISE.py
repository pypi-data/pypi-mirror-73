...
class PRECISE:
    ...
    def fit_predictor(self, X, y, alpha_values=None, l1_ratio=0.5):
        ...
        self.predictive_clf = GridSearchCV(Pipeline([
                                ('regression', Ridge() if l1_ratio == 0 else (Lasso() if self.l1_ratio == 1 else ElasticNet(self.l1_ratio)))
                                ]),\
                                cv=10,
                                n_jobs=self.n_jobs,
        ...
        self.predictive_clf.fit(self.transform(X), y)

        return self


    def compute_pred_performance(self, X, y, cv=10, with_plot=False):
        """
        ...
        for train_index, test_index in kf.split(X_projected):
            ...
            if with_plot:
                plt.plot(self.alpha_values, clf.cv_results_['mean_test_score'])
                plt.xscale('log')
                plt.show()
    ...
    def predict(self, X):
        return self.predictive_clf.predict(self.transform(X))
    ...
    def transform(self, X):
        ...
        np.ndarray of shape (n_samples, n_pv), dtype=float
            Dataset projected on consensus features.
        """
        return self.interpolation_.transform(X, self.optimal_time)
